...
function onBoardPanStart(e){
    ...
    e.preventDefault();
    closeCardMenu();
    const { scaleX, scaleY } = boardDisplayMetrics();
    panState = { startX: e.clientX, startY: e.clientY, scaleX, scaleY, ox: viewport.x, oy: viewport.y, moved: false };
    board.classList.add('panning');
}
function onBoardPanMove(e){
    if(!panState) return;
    viewport.x = panState.ox + (e.clientX - panState.startX) / panState.scaleX;
    viewport.y = panState.oy + (e.clientY - panState.startY) / panState.scaleY;
...
function openCreateCard(worldPt){
    ...
    const confirm = () => createCanvasOnBoard(input.value.trim(), createKind, placement);
