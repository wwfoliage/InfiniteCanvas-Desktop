...
const mediakitConfigBlock = document.getElementById('mediakitConfigBlock');
const mediakitKeyInput = document.getElementById('mediakitKeyInput');
const mediakitKeyState = document.getElementById('mediakitKeyState');
const mediakitKeyHint = document.getElementById('mediakitKeyHint');
const mediakitInheritOutput = document.getElementById('mediakitInheritOutput');
const mediakitCustomOutputRow = document.getElementById('mediakitCustomOutputRow');
const mediakitCustomOutput = document.getElementById('mediakitCustomOutput');
const mediakitResolvedOutput = document.getElementById('mediakitResolvedOutput');
const mediakitStatus = document.getElementById('mediakitStatus');
const mediakitSaveBtn = document.getElementById('mediakitSaveBtn');
...
const MEDIAKIT_DEFAULT_BASE_URL = 'https://mediakit.cn-beijing.volces.com/api/v1';
...
const API_PROTOCOLS = ['openai', 'apimart', 'gemini', 'grok', 'volcengine', 'runninghub', 'jimeng', 'codex', 'gemini-cli', 'mediakit'];
...
let providerDragId = '';
let mediakitSettings = null;
let mediakitSettingsLoading = null;
...
function updateIdPreview(){
    const item = provider();
    if(!item) return;
    const isBuiltin = item.id === 'comfly' || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit' || item.id === 'jimeng';
...
function isFixedProvider(itemOrId){
    ...
    return id === 'modelscope' || id === 'runninghub' || id === 'volcengine' || id === 'mediakit';
}
...
function syncEditor(){
    ...
    item.base_url = item.id === 'mediakit' ? MEDIAKIT_DEFAULT_BASE_URL : CLI_PROTOCOLS.has(selectedProtocol) ? '' : baseInput.value.trim();
    // 固定平台不从协议下拉读取
    item.protocol = selectedProtocol;
    item.image_request_mode = normalizeImageRequestMode(
        item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit' || CLI_PROTOCOLS.has(selectedProtocol)
            ? 'openai'
            : lockedApi
...
function updateProtocolFromInput(){
    const item = provider();
    if(!item || !protocolInput || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit') return;
...
function mediaKitErrorMessage(error, fallback){
    return String(error?.message || error?.detail || fallback || tr('api.mediakitSaveFailed'));
}
function syncMediaKitProviderState(settings){
    const item = providers.find(provider => provider.id === 'mediakit');
    if(!item || !settings) return;
    item.has_key = settings.configured === true;
    item.key_preview = settings.key_preview || '';
    item.key_env = 'MEDIAKIT_API_KEY';
    item.base_url = MEDIAKIT_DEFAULT_BASE_URL;
    item.protocol = 'mediakit';
    item.image_models = [];
    item.chat_models = [];
    item.video_models = [];
}
function renderMediaKitSettings(){
    const settings = mediakitSettings || {
    ...
    const followsGenerated = settings.output_mode !== 'custom';
    if(mediakitInheritOutput) mediakitInheritOutput.checked = followsGenerated;
    if(mediakitCustomOutputRow){
        mediakitCustomOutputRow.hidden = followsGenerated;
        mediakitCustomOutputRow.style.display = followsGenerated ? 'none' : 'flex';
    }
    if(mediakitCustomOutput && document.activeElement !== mediakitCustomOutput){
        mediakitCustomOutput.value = settings.custom_output_dir || '';
    }
    if(mediakitResolvedOutput) mediakitResolvedOutput.textContent = settings.resolved_output_dir || '—';
    if(mediakitKeyInput){
        mediakitKeyInput.value = '';
        mediakitKeyInput.placeholder = settings.configured
            ? `${tr('api.keepCurrentKey')} ${settings.key_preview || ''}`
            : tr('api.mediakitKeyPlaceholder');
    }
    if(mediakitKeyState){
        mediakitKeyState.textContent = settings.configured ? tr('api.mediakitConfigured') : tr('api.mediakitNotConfigured');
        mediakitKeyState.classList.toggle('ok', settings.configured === true);
    }
    if(mediakitKeyHint){
        mediakitKeyHint.textContent = settings.configured
            ? `${tr('api.keySaved')}${settings.key_preview || 'MEDIAKIT_API_KEY'}`
            : tr('api.mediakitNoKey');
    }
    syncMediaKitProviderState(settings);
    renderProviderList();
}
async function loadMediaKitSettings(force=false){
    if(mediakitSettings && !force){
        renderMediaKitSettings();
        return mediakitSettings;
    }
    if(mediakitSettingsLoading) return mediakitSettingsLoading;
    if(mediakitStatus) mediakitStatus.textContent = tr('api.loading');
    mediakitSettingsLoading = (async () => {
        const response = await fetch('/api/mediakit/settings');
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || tr('api.loadFailed'));
        mediakitSettings = data;
        renderMediaKitSettings();
        if(mediakitStatus) mediakitStatus.textContent = '';
        return data;
    })().catch(error => {
        if(mediakitStatus) mediakitStatus.textContent = mediaKitErrorMessage(error, tr('api.loadFailed'));
        return null;
    }).finally(() => {
        mediakitSettingsLoading = null;
    });
    return mediakitSettingsLoading;
}
function toggleMediaKitOutputMode(){
    const followsGenerated = mediakitInheritOutput?.checked !== false;
    if(mediakitCustomOutputRow){
        mediakitCustomOutputRow.hidden = followsGenerated;
        mediakitCustomOutputRow.style.display = followsGenerated ? 'none' : 'flex';
    }
    if(mediakitStatus) mediakitStatus.textContent = followsGenerated ? tr('api.mediakitFollowGeneratedHint') : tr('api.mediakitCustomHint');
}
async function saveMediaKitSettings(){
    const key = mediakitKeyInput?.value.trim() || '';
    const followsGenerated = mediakitInheritOutput?.checked !== false;
    const payload = {
        output_mode:followsGenerated ? 'generated' : 'custom',
        custom_output_dir:followsGenerated ? (mediakitSettings?.custom_output_dir || '') : (mediakitCustomOutput?.value.trim() || '')
    };
    if(key) payload.api_key = key;
    if(mediakitSaveBtn) mediakitSaveBtn.disabled = true;
    if(mediakitStatus) mediakitStatus.textContent = tr('api.saving');
    try {
        const response = await fetch('/api/mediakit/settings', {
            method:'PATCH',
            headers:{'Content-Type':'application/json'},
        ...
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || tr('api.mediakitSaveFailed'));
        mediakitSettings = data;
        renderMediaKitSettings();
        if(mediakitStatus) mediakitStatus.textContent = tr('api.mediakitSaved');
        if(mediakitKeyInput) mediakitKeyInput.value = '';
        broadcastStudioApiChange('mediakit-settings-changed');
        return true;
    } catch(error) {
        if(mediakitStatus) mediakitStatus.textContent = mediaKitErrorMessage(error);
        return false;
    } finally {
        if(mediakitSaveBtn) mediakitSaveBtn.disabled = false;
    }
}
async function clearMediaKitKey(){
    if(!mediakitSettings?.configured && !mediakitKeyInput?.value) return;
    if(!confirm(tr('api.confirmClearKey'))) return;
    const payload = {
        clear_api_key:true,
        output_mode:mediakitInheritOutput?.checked === false ? 'custom' : 'generated',
        custom_output_dir:mediakitCustomOutput?.value.trim() || mediakitSettings?.custom_output_dir || ''
    };
    try {
        const response = await fetch('/api/mediakit/settings', {
            method:'PATCH',
        ...
        mediakitSettings = data;
        renderMediaKitSettings();
        if(mediakitStatus) mediakitStatus.textContent = tr('api.mediakitKeyCleared');
        broadcastStudioApiChange('mediakit-settings-changed');
    } catch(error) {
        if(mediakitStatus) mediakitStatus.textContent = mediaKitErrorMessage(error);
    }
}
async function openMediaKitOutputDir(){
    if(mediakitStatus) mediakitStatus.textContent = tr('api.mediakitOpeningOutput');
    try {
        const response = await fetch('/api/mediakit/open-output-dir', {method:'POST'});
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || tr('api.mediakitOpenFailed'));
        if(mediakitStatus) mediakitStatus.textContent = tr('api.mediakitOpenedOutput');
    } catch(error) {
        if(mediakitStatus) mediakitStatus.textContent = mediaKitErrorMessage(error, tr('api.mediakitOpenFailed'));
    }
}
function sortedProviders(){
    const order = ['modelscope', 'runninghub', 'volcengine', 'mediakit'];
...
function renderProviderList(){
    providerList.innerHTML = sortedProviders().map(item => {
        ...
        if(item.id === 'mediakit'){
            return `
                <button class="provider-card provider-card-mediakit ${active} ${stateClass}" type="button" onclick="selectProvider('${escapeHtml(item.id)}')">
                    <span class="provider-mark provider-mark-mediakit"><i data-lucide="wand-sparkles" class="w-4 h-4"></i></span>
                    <span class="provider-info">
                        <span class="provider-name">AI MediaKit</span>
                        <span class="provider-meta">${escapeHtml(tr('api.mediakitProviderMeta'))}</span>
                    </span>
                    <span class="provider-side-meta">
                        <span class="provider-status-dot"></span>
                        <span class="provider-protocol-pill">MediaKit</span>
                    </span>
                </button>
            `;
        }
...
function renderEditor(){
    ...
    if(protocolInput){
        protocolInput.value = item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'mediakit' ? 'mediakit' : (item.protocol || 'openai');
    ...
    if(imageRequestModeInput){
        ...
        imageRequestModeInput.disabled = Boolean(lockedApi) || item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit' || CLI_PROTOCOLS.has(String(protocolInput?.value || item.protocol || '').toLowerCase());
    ...
    if(imageEditRouteInput){
        ...
        imageEditRouteInput.disabled = item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit' || CLI_PROTOCOLS.has(String(protocolInput?.value || item.protocol || '').toLowerCase());
    }
    keyInput.value = '';
    ...
    const isMediaKit = item.id === 'mediakit';
    ...
    document.body.classList.toggle('show-mediakit', isMediaKit);
    ...
    if(mediakitConfigBlock){
        mediakitConfigBlock.hidden = !isMediaKit;
        mediakitConfigBlock.style.display = isMediaKit ? 'flex' : 'none';
        if(isMediaKit) loadMediaKitSettings();
    }
    if(!isRunningHub){
        if(rhPasteInput) rhPasteInput.value = '';
        if(rhAppsList) rhAppsList.innerHTML = '';
...
const FIXED_PROTOCOL_PROVIDER_IDS = new Set(['modelscope', 'volcengine', 'runninghub', 'mediakit']);
...
async function saveProviders(){
    ...
    providers.forEach(item => {
        ...
        item.image_request_mode = normalizeImageRequestMode(
            item.id === 'modelscope' || item.id === 'runninghub' || item.id === 'volcengine' || item.id === 'mediakit' || isCliProtocol
                ? 'openai'
    ...
    try {
        const res = await fetch('/api/providers', {
            ...
            body:JSON.stringify(providers.map(item => ({
                ...
                protocol:(item.id === 'modelscope') ? 'openai' : item.id === 'runninghub' ? 'runninghub' : item.id === 'volcengine' ? 'volcengine' : item.id === 'mediakit' ? 'mediakit' : (item.protocol || 'openai'),
