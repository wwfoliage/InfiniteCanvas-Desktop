...
const mediaKitComposerPanel = document.getElementById('mediaKitComposerPanel');
...
const smartSnapToggle = document.getElementById('smartSnapToggle');
...
const smartMediaKitPollers = new Map();
...
function smartVideoPreviewHtml(itemOrUrl, size=512, attrs=''){
    ...
    return `<video src="${escapeHtml(src)}" data-original-src="${escapeAttr(original)}" data-url="${escapeAttr(original)}" data-preview-kind="video" muted preload="auto" playsinline disablepictureinpicture controlslist="nodownload noplaybackrate noremoteplayback"${attrs ? ` ${attrs}` : ''}></video>`;
}
...
function smartMediaPlaybackIdentity(media){
    if(!media) return '';
    const nodeEl = media.closest?.('.image-node');
    const itemEl = media.closest?.('[data-image-index]');
    const tag = media.tagName?.toLowerCase?.() || 'media';
    const nodeId = nodeEl?.dataset?.id || '';
    const refNodeId = itemEl?.dataset?.refNodeId || nodeId;
    const imageIndex = itemEl?.dataset?.refImageIndex ?? itemEl?.dataset?.imageIndex ?? '0';
    const url = media.dataset?.url || media.getAttribute?.('src') || '';
    return `${tag}:${nodeId}:${refNodeId}:${imageIndex}:${url}`;
}
function pauseOtherCanvasMedia(activeMedia){
...
function bindSmartVideoPlaybackUi(video){
    if(!video || video.dataset.playbackUiBound === '1') return;
    video.dataset.playbackUiBound = '1';
    const sync = () => {
        const root = video.closest('.media-video-card,.video-thumb');
...
function smartActivateVideoPreview(target){
    ...
    const video = target?.matches?.('video[data-url]') ? target : root?.querySelector?.('video[data-url]');
...
function isSmartVideoEnhanceNode(node){
    return Boolean(node && node.type === 'smart-video-enhance');
}
function isSmartMediaKitResultNode(node){
    return Boolean(isSmartImageNode(node) && node.mediakitRole === 'result');
}
function isSmartMediaKitControllerNode(node){
    return Boolean(isSmartVideoEnhanceNode(node) || isSmartMediaKitResultNode(node));
}
...
function imageLayout(images, scale=1, node=null){
    ...
    if(node?.type === 'smart-video-enhance'){
        return {
            cols:1,
            rows:1,
            width:Math.round(Number(node.w) || 348),
            height:Math.round(Number(node.h) || 404),
            thumb:96,
            single:true
        };
    }
...
function isSeedance2VideoModel(model){
    ...
    return /seedance(?:2[\s._/-]*0|[\s._/-]+2[\s._/-]+0)(?!\d)/.test(normalized);
}
function isTudouVideoProvider(providerId){
    const provider = videoProviderById(providerId);
    const baseUrl = String(provider?.base_url || '').trim().toLowerCase();
    try {
        const host = new URL(baseUrl).hostname.toLowerCase();
        return host === 'api.ai-tudou.net' || host.endsWith('.ai-tudou.net');
    } catch(_) {
        return false;
    }
}
function supportsSeedance2Video480p(target=settings){
    if(!isSeedance2VideoModel(target?.videoModel)) return false;
    const engine = String(target?.engine || '').trim().toLowerCase();
    const provider = String(target?.videoProvider || '').trim().toLowerCase();
    if(engine === 'volcengine' || provider === 'volcengine') return true;
    if(engine !== 'api' || isJimengProviderId(provider) || isTudouVideoProvider(provider)) return false;
    const protocol = String(videoProviderById(provider)?.protocol || 'openai').trim().toLowerCase();
    return protocol === 'openai' || protocol === 'apimart' || protocol === 'volcengine';
}
function sanitizeVideoResolutionForSelection(target=settings){
    if(target?.videoResolution === '480p' && !supportsSeedance2Video480p(target)){
...
function sanitizeSmartApiSelection(target=settings){
    ...
    if(target.engine === 'volcengine'){
        if(target.apiKind === 'video'){
            ...
            sanitizeVideoResolutionForSelection(target);
        } else {
...
function renderVideoResolutionControl(){
    sanitizeVideoResolutionForSelection(settings);
    const options = [['', tr('smart.videoResAuto')]];
    if(supportsSeedance2Video480p(settings)) options.push(['480p','480P']);
    options.push(['720p','720P'], ['1080p','1080P'], ['4k','4K']);
...
function setDynamicSetting(key, value){
    ...
    if(key === 'videoProvider' || key === 'videoModel') sanitizeVideoResolutionForSelection(settings);
...
function createSmartVideoEnhanceNode(x, y, options={}){
    if(!options.skipUndo) pushUndo();
    const node = {
        id:uid('enhance'),
    ...
    nodes.push(node);
    if(options.connectFromId) connectInputNode(options.connectFromId, node.id);
    if(options.select !== false) selectedId = node.id;
    render();
    scheduleSave();
    return node;
...
function captureMediaPlaybackStates(){
    ...
    world.querySelectorAll('video[data-url], audio[data-url]').forEach(media => {
        const key = smartMediaPlaybackIdentity(media);
...
function restoreMediaPlaybackStates(states){
    ...
    world.querySelectorAll('video[data-url], audio[data-url]').forEach(media => {
        restoreMediaPlaybackState(media, states.get(smartMediaPlaybackIdentity(media)));
    });
}
...
function smartMediaKitSourceEntries(node){
    if(!node) return [];
    if(node.mediakitBatchSource?.url){
        return [{sourceNode:nodes.find(item => item.id === node.mediakitSourceNodeId) || null, item:{...node.mediakitBatchSource}}];
    }
    const seen = new Set();
    return inputNodesFor(node).flatMap(sourceNode => imagesForNode(sourceNode).map(source => ({sourceNode, item:imageForDisplay(source)})))
        .filter(entry => entry.item?.url && mediaKindForItem(entry.item) === 'video')
        .filter(entry => {
            const key = String(entry.item.url || '');
            if(!key || seen.has(key)) return false;
            seen.add(key);
            return true;
        });
}
function smartMediaKitSourceItems(node){
    return smartMediaKitSourceEntries(node).map(entry => entry.item);
}
function smartMediaKitSourceItem(node){
    const items = smartMediaKitSourceItems(node);
    return items.length === 1 ? items[0] : null;
}
function smartMediaKitStatusText(node){
    const status = String(node?.mediakitStatus || 'idle');
    if(node?.mediakitMessage) return node.mediakitMessage;
    if(status === 'uploading') return tr('smart.mediakitUploading');
    if(status === 'submitted' || status === 'processing') return tr('smart.mediakitProcessing');
    if(status === 'saving') return tr('smart.mediakitSaving');
    if(status === 'succeeded') return tr('smart.mediakitSucceeded');
    if(status === 'save_failed') return tr('smart.mediakitSaveFailed');
    if(status === 'failed') return tr('smart.mediakitFailed');
    return tr('smart.mediakitIdle');
}
function smartMediaKitEnhanceBodyHtml(node, options={}){
    const sourceItems = smartMediaKitSourceItems(node);
    const resolution = ['720p','1080p','2k'].includes(node.resolution) ? node.resolution : '720p';
    const bitrate = ['low','medium','high'].includes(node.bitrateLevel) ? node.bitrateLevel : 'medium';
    const keepSourceFps = node.keepSourceFps !== false;
    const busy = Boolean(node.running || ['uploading','submitted','processing','saving'].includes(String(node.mediakitStatus || '')));
    const hasOutput = isSmartMediaKitResultNode(node) && (node.images || []).some(item => item?.url && mediaKindForItem(imageForDisplay(item)) === 'video');
    const startLabel = hasOutput
        ? (window.StudioI18n?.lang?.() === 'en' ? 'Enhance again' : '重新增强')
        : tr('smart.mediakitStart');
    const statusClass = node.mediakitError ? ' error' : String(node.mediakitStatus || '') === 'succeeded' ? ' success' : '';
    const videoLabelPrefix = window.StudioI18n?.lang?.() === 'en' ? 'Video ' : '视频';
    const sourceHtml = sourceItems.length
        ? smartNodeInputThumbsHtml(sourceItems, {labelPrefix:videoLabelPrefix})
        : `<div class="smart-mediakit-source-empty"><i data-lucide="video"></i><span>${escapeHtml(tr('smart.mediakitConnectVideo'))}</span></div>`;
    const segment = (field, values, current) => `<div class="smart-mediakit-segment">${values.map(([value,label]) => `
        <button type="button" data-mediakit-field="${field}" data-mediakit-value="${value}" class="${current === value ? 'active' : ''}" ${busy ? 'disabled' : ''}>${escapeHtml(label)}</button>`).join('')}</div>`;
    return `<div class="smart-mediakit-card">
        <div class="smart-mediakit-heading"><i data-lucide="wand-sparkles"></i><strong>${escapeHtml(tr('smart.mediakitTitle'))}</strong><span>MediaKit</span></div>
        ${sourceHtml}
        <div class="smart-mediakit-control">
            <span>${escapeHtml(tr('smart.mediakitResolution'))}</span>
            ${segment('resolution', [['720p','720P'],['1080p','1080P'],['2k','2K']], resolution)}
        </div>
        <div class="smart-mediakit-control">
            <span>${escapeHtml(tr('smart.mediakitBitrate'))}</span>
            ${segment('bitrateLevel', [['low',tr('smart.mediakitLow')],['medium',tr('smart.mediakitMedium')],['high',tr('smart.mediakitHigh')]], bitrate)}
        </div>
        <label class="smart-mediakit-fps-toggle">
            <input type="checkbox" data-mediakit-keep-fps="1" ${keepSourceFps ? 'checked' : ''} ${busy ? 'disabled' : ''}>
            <span><strong>${escapeHtml(tr('smart.mediakitKeepFps'))}</strong><small>${escapeHtml(tr('smart.mediakitKeepFpsHint'))}</small></span>
        </label>
        <label class="smart-mediakit-fps-input ${keepSourceFps ? 'is-hidden' : ''}">
            <span>${escapeHtml(tr('smart.mediakitTargetFps'))}</span>
            <input type="number" min="15" max="120" step="1" value="${Math.max(15, Math.min(120, Number(node.fps || 30)))}" data-mediakit-fps="1" ${busy ? 'disabled' : ''}>
        </label>
        <div class="smart-mediakit-status${statusClass}">
            <i data-lucide="${busy ? 'loader-2' : node.mediakitError ? 'circle-alert' : 'info'}"></i>
            <span>${escapeHtml(node.mediakitError || smartMediaKitStatusText(node))}</span>
        </div>
        <div class="smart-mediakit-actions">
            <button type="button" class="smart-mediakit-run" data-mediakit-start="1" ${busy || !sourceItems.length ? 'disabled' : ''}>
                <i data-lucide="${busy ? 'loader-2' : 'wand-sparkles'}"></i>
                <span>${escapeHtml(busy ? tr('smart.mediakitRunning') : startLabel)}</span>
            </button>
            ${String(node.mediakitStatus || '') === 'save_failed' && node.mediakitTaskId ? `<button type="button" class="smart-mediakit-retry" data-mediakit-retry-save="1"><i data-lucide="download"></i><span>${escapeHtml(tr('smart.mediakitRetrySave'))}</span></button>` : ''}
        </div>
    </div>`;
}
function smartMediaKitResultErrorHtml(node, layout){
    const canRetry = node.mediakitStatus === 'save_failed' && node.mediakitTaskId;
    const canRetryBatch = Boolean(node.mediakitBatchSource?.url);
    return `<div class="smart-mediakit-result-error" style="width:${layout.width}px;height:${layout.height}px">
        <i data-lucide="circle-alert"></i>
        <strong>${escapeHtml(tr('smart.mediakitResultFailed'))}</strong>
        <span>${escapeHtml(node.mediakitError || tr('smart.mediakitFailed'))}</span>
        ${canRetry ? `<button type="button" data-mediakit-result-retry="${escapeAttr(node.id)}"><i data-lucide="download"></i><span>${escapeHtml(tr('smart.mediakitRetrySave'))}</span></button>` : ''}
        ${!canRetry && canRetryBatch ? `<button type="button" data-mediakit-batch-retry="${escapeAttr(node.id)}"><i data-lucide="refresh-cw"></i><span>${escapeHtml(window.StudioI18n?.lang?.() === 'en' ? 'Retry this video' : '重试此视频')}</span></button>` : ''}
    </div>`;
}
function smartMediaKitPendingBodyHtml(node, layout){
    const text = window.StudioI18n?.lang?.() === 'en'
        ? 'Set the enhancement options below, then click Start enhancement.'
        : '在下方设置画质增强参数，然后点击开始增强';
    return `<div class="smart-mediakit-pending" style="width:${layout.width}px;height:${layout.height}px">
        <i data-lucide="wand-sparkles"></i>
        <strong>${escapeHtml(tr('smart.mediakitResultTitle'))}</strong>
...
function nodeBodyHtml(node, layout){
    ...
    if(node.type === 'smart-video-enhance') return smartMediaKitEnhanceBodyHtml(node);
    if(node.mediakitError && !(node.images || []).length) return smartMediaKitResultErrorHtml(node, layout);
    if(isSmartMediaKitResultNode(node) && !(node.images || []).length && !node.pending) return smartMediaKitPendingBodyHtml(node, layout);
...
function smartRecoverableImageTask(node){
    return smartPendingTasks(node).find(task => ['reconnecting','manual'].includes(String(task.status || '')) || (task.failed && task.recoverTaskId)) || null;
}
function imageTaskRecoverBodyHtml(node, task, layout){
    const querying = Boolean(task.querying);
    const recoverableCount = smartPendingTasks(node).filter(item => ['reconnecting','manual'].includes(String(item.status || '')) || (item.failed && item.recoverTaskId)).length;
    const reconnecting = String(task.status || '') === 'reconnecting';
    const title = querying ? '查询中' : reconnecting ? '网络波动，正在自动重连' : '已停止自动查询';
    const sub = recoverableCount > 1 ? `还有 ${recoverableCount} 个任务可查询` : `本地任务：${task.taskId || ''}`;
    return `<div class="jimeng-pending-cell loading-cell single" style="width:${layout.width}px;height:${layout.height}px">
        <div class="jimeng-pending-overlay">
            ...
            <button class="jimeng-pending-query" type="button" data-media-task-query="${escapeAttr(node.id)}" data-task-id="${escapeAttr(task.taskId)}" ${querying ? 'disabled' : ''}><i data-lucide="${querying ? 'loader-2' : 'refresh-cw'}"></i><span>${querying ? '查询中…' : '立即查询'}</span></button>
        </div>
    </div>`;
}
...
function smartNodeToolbarHtml(node){
    ...
    const actions = [
        {key:'mediakit-enhance', icon:'wand-sparkles', label:tr('smart.mediakitToolbar'), enabled:kind === 'video'},
...
function runSmartNodeToolbarAction(nodeId, action){
    ...
    if(action === 'mediakit-enhance'){
        if(kind !== 'video'){
            toast(tr('smart.mediakitConnectVideo'));
            return;
        }
        createSmartMediaKitResultNode(node);
        return;
    }
...
function createSmartMediaKitResultNode(sourceNode, options={}){
    if(!options.skipUndo) pushUndo();
    const pendingBox = {w:520, h:320};
    const point = options.point || null;
    const pos = sourceNode
        ? nextOutputPositionForSource(sourceNode, pendingBox)
        : point
        ? {x:Number(point.x || 0) - pendingBox.w / 2, y:Number(point.y || 0) - pendingBox.h / 2}
        : {x:0, y:0};
    const result = {
        id:uid('smart'),
        type:'smart-image',
        ...
        w:pendingBox.w,
        h:pendingBox.h,
        title:tr('smart.mediakitResultTitle'),
        images:[],
        pending:0,
        running:false,
        outputKind:'video',
        ...
        mediakitRole:'result',
        mediakitSourceNodeId:sourceNode?.id || '',
        mediakitBatchSource:options.mediakitBatchSource ? {...options.mediakitBatchSource} : null,
        mediakitBatchGroupId:options.mediakitBatchGroupId || '',
        mediakitBatchParentId:options.mediakitBatchParentId || '',
        resolution:options.resolution || '720p',
        bitrateLevel:options.bitrateLevel || 'medium',
        keepSourceFps:options.keepSourceFps !== false,
        fps:Math.max(15, Math.min(120, Math.round(Number(options.fps || 30)))),
        mediakitStatus:'idle',
        mediakitMessage:'',
        mediakitError:'',
        created_at:Date.now()
    };
    if(options.legacy) delete result.mediakitRole;
    if(options.status) result.mediakitStatus = options.status;
    nodes.push(result);
    if(sourceNode) connectInputNode(sourceNode.id, result.id);
...
function smartMediaKitResultNode(controller){
    if(isSmartMediaKitResultNode(controller)) return controller;
    const explicit = nodes.find(node => node.id === controller?.mediakitResultNodeId);
    if(explicit) return explicit;
    return (canvas?.connections || [])
        .filter(conn => conn.from === controller?.id)
        .map(conn => nodes.find(node => node.id === conn.to))
        .find(node => isSmartImageNode(node) && node.mediakitTaskId === controller?.mediakitTaskId) || null;
}
function collectSmartMediaKitBatchResult(resultNode){
    if(!resultNode?.mediakitBatchGroupId || resultNode.mediakitStatus !== 'succeeded' || !(resultNode.images || []).length) return false;
    const group = nodes.find(node => node.id === resultNode.mediakitBatchGroupId && isSmartGroupNode(node));
    if(!group) return false;
    const collected = addNodeToSmartGroup(group, resultNode);
    if(collected) arrangeSmartGroupMembers(group, {skipUndo:true});
    return collected;
}
function applySmartMediaKitTask(controllerId, task){
    const controller = nodes.find(node => node.id === controllerId);
    if(!controller || !task) return;
    const resultNode = nodes.find(node => node.id === (task.result_node_id || controller.mediakitResultNodeId))
        || smartMediaKitResultNode(controller);
    const status = task.status || controller.mediakitStatus || 'processing';
    const busy = ['uploading','submitted','processing','saving'].includes(String(status));
    controller.mediakitTaskId = task.task_id || controller.mediakitTaskId || '';
    controller.mediakitStatus = status;
    controller.mediakitMessage = task.message || '';
    controller.mediakitError = task.error || '';
    controller.running = busy;
    if(resultNode){
        resultNode.mediakitTaskId = task.task_id || resultNode.mediakitTaskId || '';
        resultNode.mediakitStatus = status;
        resultNode.mediakitMessage = task.message || '';
        resultNode.mediakitError = task.error || '';
        resultNode.running = busy;
        if(status === 'succeeded' && task.result?.url){
            const output = {
                url:task.result.url,
                localUrl:task.result.url,
                savedUrl:task.result.saved_url || task.result.url,
                name:task.result.name || 'mediakit-enhanced.mp4',
                kind:'video',
                generatedResult:true,
                mediakitTaskId:task.task_id || ''
            };
            copyMediaSizeFields(task.result, output);
            if(Number(task.result.duration) > 0) output.duration = Number(task.result.duration);
            resultNode.images = [output];
            resultNode.pending = 0;
            resultNode.running = false;
            resultNode.title = tr('smart.mediakitResultTitle');
            resultNode.runFinishedAt = nowMs();
            resultNode.runElapsedMs = Math.max(0, resultNode.runFinishedAt - Number(resultNode.runStartedAt || resultNode.runFinishedAt));
            resultNode.runTimerHidden = false;
            delete resultNode.w;
            delete resultNode.h;
        } else if(status === 'failed' || status === 'save_failed'){
            resultNode.pending = 0;
            resultNode.running = false;
        } else {
            resultNode.pending = Math.max(1, Number(resultNode.pending || 1));
        }
    }
    if(status === 'succeeded'){
        controller.running = false;
        controller.runFinishedAt = resultNode?.runFinishedAt || nowMs();
        controller.runElapsedMs = Math.max(0, controller.runFinishedAt - Number(controller.runStartedAt || controller.runFinishedAt));
    }
    render();
    scheduleSave();
}
async function pollSmartMediaKitTask(controllerId, taskId){
    if(!window.MediaKitClient || !controllerId || !taskId) return;
    const key = `${controllerId}:${taskId}`;
    if(smartMediaKitPollers.has(key)) return smartMediaKitPollers.get(key).promise;
    const abortController = new AbortController();
    const promise = window.MediaKitClient.pollTask(taskId, {
        signal:abortController.signal,
        onUpdate:task => applySmartMediaKitTask(controllerId, task)
    }).then(task => {
        applySmartMediaKitTask(controllerId, task);
        const resultNode = nodes.find(node => node.id === controllerId);
        if(collectSmartMediaKitBatchResult(resultNode)){
            render();
            scheduleSave();
        }
        return task;
        ...
        if(node){
            node.mediakitError = window.MediaKitClient.errorMessage(error, tr('smart.mediakitQueryFailed'));
            node.mediakitStatus = node.mediakitStatus || 'processing';
            render();
            scheduleSave();
            setTimeout(() => {
                const retryNode = nodes.find(item => item.id === controllerId);
                if(retryNode?.mediakitTaskId === taskId && !window.MediaKitClient.TERMINAL_STATUSES.has(String(retryNode.mediakitStatus || ''))){
                    pollSmartMediaKitTask(controllerId, taskId);
                }
            }, 5000);
        }
        return null;
    }).finally(() => {
        smartMediaKitPollers.delete(key);
    });
    smartMediaKitPollers.set(key, {controller:abortController, promise});
    return promise;
}
async function runSmartMediaKitBatch(controller, entries){
    if(!controller || !entries?.length || !window.MediaKitClient) return null;
    try {
        const settingsState = await window.MediaKitClient.getSettings();
        if(!settingsState.configured) throw new Error(tr('smart.mediakitConfigureKey'));
    } catch(error) {
        controller.mediakitError = window.MediaKitClient.errorMessage(error, tr('smart.mediakitFailed'));
        controller.mediakitStatus = 'failed';
        controller.running = false;
        render();
        scheduleSave();
        return null;
    ...
    pushUndo();
    const rect = nodeRect(controller);
    const group = createSmartGroupNode(rect.x + rect.width + 220, rect.y, {skipUndo:true, select:false});
    group.title = window.StudioI18n?.lang?.() === 'en' ? 'Enhanced videos' : '画质增强结果';
    group.mediakitBatchGroup = true;
    controller.mediakitBatchGroupId = group.id;
    controller.mediakitBatchItems = [];
    controller.mediakitStatus = 'processing';
    controller.mediakitError = '';
    controller.running = true;
    controller.runStartedAt = nowMs();
    let succeeded = 0;
    let failed = 0;
    let index = 0;
    for(const entry of entries){
        ...
        controller.mediakitMessage = window.StudioI18n?.lang?.() === 'en'
            ? `Enhancing video ${index} of ${entries.length}`
            : `正在增强第 ${index}/${entries.length} 个视频`;
        const resultNode = createSmartMediaKitResultNode(entry.sourceNode, {
            skipUndo:true,
            select:false,
            render:false,
            status:'submitting',
            resolution:controller.resolution,
            bitrateLevel:controller.bitrateLevel,
            keepSourceFps:controller.keepSourceFps,
            fps:controller.fps,
            mediakitBatchSource:source,
            mediakitBatchGroupId:group.id,
            mediakitBatchParentId:controller.id
        });
        resultNode.title = `${tr('smart.mediakitResultTitle')} ${index}`;
        const batchItem = {index, sourceUrl:source.url, sourceName:source.name || `video-${index}.mp4`, resultNodeId:resultNode.id, status:'processing', error:''};
        controller.mediakitBatchItems.push(batchItem);
        render();
        scheduleSave();
        const task = await runSmartMediaKitSource(resultNode, source);
        batchItem.taskId = task?.task_id || resultNode.mediakitTaskId || '';
        batchItem.status = task?.status || resultNode.mediakitStatus || 'failed';
        batchItem.error = resultNode.mediakitError || '';
        if(batchItem.status === 'succeeded') succeeded += 1;
        else failed += 1;
        controller.mediakitMessage = window.StudioI18n?.lang?.() === 'en'
            ? `${succeeded} succeeded, ${failed} failed, ${entries.length - index} remaining`
            : `已成功 ${succeeded} 个，失败 ${failed} 个，剩余 ${entries.length - index} 个`;
        render();
        scheduleSave();
        await saveCanvas();
    }
    controller.running = false;
    controller.runFinishedAt = nowMs();
    controller.runElapsedMs = Math.max(0, controller.runFinishedAt - Number(controller.runStartedAt || controller.runFinishedAt));
    controller.mediakitStatus = failed ? (succeeded ? 'partial' : 'failed') : 'succeeded';
    controller.mediakitMessage = window.StudioI18n?.lang?.() === 'en'
    ...
    scheduleSave();
    await saveCanvas();
    return {succeeded, failed, groupId:group.id};
}
async function runSmartMediaKitSource(controller, source){
    if(!controller || !source?.url) return null;
    return startSmartMediaKitEnhance(controller.id, {sourceOverride:source, batchChild:true});
}
async function startSmartMediaKitEnhance(nodeId, options={}){
    const controller = nodes.find(item => item.id === nodeId);
    if(!isSmartMediaKitControllerNode(controller) || !window.MediaKitClient) return;
    const sourceEntries = options.sourceOverride
        ? [{sourceNode:nodes.find(item => item.id === controller.mediakitSourceNodeId) || null, item:options.sourceOverride}]
        : smartMediaKitSourceEntries(controller);
    if(sourceEntries.length > 1 && isSmartVideoEnhanceNode(controller) && !options.batchChild){
        return runSmartMediaKitBatch(controller, sourceEntries);
    }
    const sources = sourceEntries.map(entry => entry.item);
    if(sources.length !== 1){
        controller.mediakitError = tr('smart.mediakitConnectVideo');
        render();
        return;
    }
    try {
        const enhanceParams = window.MediaKitClient.validateParams({
            resolution:controller.resolution || '720p',
            bitrate_level:controller.bitrateLevel || 'medium',
            fps:controller.keepSourceFps === false ? controller.fps : null,
            keepSourceFps:controller.keepSourceFps !== false
        });
        const settingsState = await window.MediaKitClient.getSettings();
        if(!settingsState.configured) throw new Error(tr('smart.mediakitConfigureKey'));
        let resultNode = smartMediaKitResultNode(controller);
        if(isSmartVideoEnhanceNode(controller)){
            pushUndo();
            resultNode = createSmartMediaKitResultNode(controller, {
                legacy:true,
                skipUndo:true,
                select:false,
                render:false,
        ...
        if(!resultNode) throw new Error(tr('smart.mediakitFailed'));
        const canReuseClientToken = controller.mediakitStatus === 'failed'
            && !controller.mediakitTaskId
            && Boolean(controller.mediakitClientToken)
            && /响应缺少 result|未返回任务 ID|未返回 task_id/.test(String(controller.mediakitError || ''));
        const clientToken = canReuseClientToken
            ? controller.mediakitClientToken
            : window.MediaKitClient.createClientToken();
        controller.mediakitClientToken = clientToken;
        controller.mediakitResultNodeId = resultNode.id;
        controller.mediakitTaskId = '';
        controller.mediakitStatus = 'uploading';
        controller.mediakitMessage = tr('smart.mediakitPreparing');
        controller.mediakitError = '';
        controller.running = true;
        controller.runStartedAt = nowMs();
        controller.runTimerHidden = isSmartVideoEnhanceNode(controller);
        delete controller.runFinishedAt;
        resultNode.pending = 1;
        resultNode.running = true;
        resultNode.runStartedAt = controller.runStartedAt;
        resultNode.runTimerHidden = false;
        resultNode.mediakitStatus = 'uploading';
        resultNode.mediakitMessage = tr('smart.mediakitPreparing');
        resultNode.mediakitError = '';
        selectedId = resultNode.id;
        selectedIds = [];
        selectedImage = {nodeId:'', index:-1};
        render();
        scheduleSave();
        ...
        let metadata = {
            width:Number(source.natural_w || source.width || source.w || 0) || null,
            height:Number(source.natural_h || source.height || source.h || 0) || null,
            duration:Number(source.duration || 0) || null
        };
        if(!metadata.width || !metadata.height){
            try {
                metadata = await Promise.race([
                    window.MediaKitClient.videoMetadata(source.url),
                    new Promise(resolve => setTimeout(() => resolve(metadata), 5000))
                ]);
            } catch(_) {}
        }
        const task = await window.MediaKitClient.createEnhanceTask({
            video:{
                url:source.url,
            ...
            canvas_id:canvasId,
            enhance_node_id:controller.id,
            result_node_id:resultNode.id
        });
        controller.mediakitTaskId = task.task_id || '';
        resultNode.mediakitTaskId = task.task_id || '';
        applySmartMediaKitTask(controller.id, task);
        await saveCanvas();
        let terminalTask = task;
        if(task.task_id && !window.MediaKitClient.TERMINAL_STATUSES.has(String(task.status || ''))){
            terminalTask = await pollSmartMediaKitTask(controller.id, task.task_id) || task;
        }
        const liveResult = nodes.find(item => item.id === resultNode.id);
        if(liveResult && collectSmartMediaKitBatchResult(liveResult)){
            render();
            scheduleSave();
            await saveCanvas();
        }
        ...
        const live = nodes.find(item => item.id === nodeId);
        if(!live) return;
        live.running = false;
        live.mediakitStatus = 'failed';
        live.mediakitMessage = '';
        live.mediakitError = window.MediaKitClient.errorMessage(error, tr('smart.mediakitFailed'));
        const result = smartMediaKitResultNode(live);
        if(result){
            result.pending = 0;
            result.mediakitStatus = 'failed';
            result.mediakitError = live.mediakitError;
        }
        render();
        scheduleSave();
        await saveCanvas();
...
async function retrySmartMediaKitSave(nodeId){
    if(!window.MediaKitClient) return;
    const node = nodes.find(item => item.id === nodeId);
    const controller = isSmartMediaKitControllerNode(node)
        ? node
        : nodes.find(item => isSmartVideoEnhanceNode(item) && item.mediakitResultNodeId === nodeId);
    const taskId = node?.mediakitTaskId || controller?.mediakitTaskId;
    if(!controller || !taskId) return;
    controller.running = true;
    controller.mediakitStatus = 'saving';
    controller.mediakitError = '';
    render();
    try {
        const task = await window.MediaKitClient.retrySave(taskId);
        applySmartMediaKitTask(controller.id, task);
    } catch(error) {
        controller.running = false;
        controller.mediakitStatus = 'save_failed';
        controller.mediakitError = window.MediaKitClient.errorMessage(error, tr('smart.mediakitSaveFailed'));
        render();
        scheduleSave();
    }
}
function bindSmartMediaKitControls(el, node){
    if(!isSmartMediaKitControllerNode(node)) return;
    el.querySelectorAll('[data-mediakit-field]').forEach(button => {
        button.addEventListener('click', event => {
            event.preventDefault();
            event.stopPropagation();
            if(node.running) return;
            node[button.dataset.mediakitField] = button.dataset.mediakitValue;
            node.mediakitError = '';
            render();
            scheduleSave();
        });
    });
    const keepFps = el.querySelector('[data-mediakit-keep-fps]');
    keepFps?.addEventListener('change', event => {
        event.stopPropagation();
        if(node.running) return;
        node.keepSourceFps = keepFps.checked;
        node.mediakitError = '';
        render();
        scheduleSave();
    });
    const fpsInput = el.querySelector('[data-mediakit-fps]');
    fpsInput?.addEventListener('change', event => {
        event.stopPropagation();
        if(node.running) return;
        node.fps = Math.round(Number(fpsInput.value || 30));
        scheduleSave();
    });
    el.querySelector('[data-mediakit-start]')?.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        if(node.running) return;
        if(isSmartMediaKitResultNode(node) && (node.images || []).some(item => item?.url && mediaKindForItem(imageForDisplay(item)) === 'video')){
            pushUndo();
            const nextResult = createSmartMediaKitResultNode(node, {
                skipUndo:true,
                render:false,
                resolution:node.resolution,
                bitrateLevel:node.bitrateLevel,
                keepSourceFps:node.keepSourceFps,
                fps:node.fps
            });
            render();
            scheduleSave();
            startSmartMediaKitEnhance(nextResult.id);
            return;
        }
        startSmartMediaKitEnhance(node.id);
    });
    el.querySelector('[data-mediakit-retry-save]')?.addEventListener('click', event => {
        event.preventDefault();
        event.stopPropagation();
        retrySmartMediaKitSave(node.id);
    });
}
function resumeSmartMediaKitTasks(){
    if(!window.MediaKitClient) return;
    nodes.filter(isSmartMediaKitControllerNode).forEach(node => {
        const taskId = String(node.mediakitTaskId || '');
        const result = smartMediaKitResultNode(node);
        const alreadyComplete = node.mediakitStatus === 'succeeded' && result?.images?.some(item => item?.url);
        const terminal = window.MediaKitClient.TERMINAL_STATUSES.has(String(node.mediakitStatus || ''));
        if(taskId && !alreadyComplete && !terminal) pollSmartMediaKitTask(node.id, taskId);
    });
}
async function runJimengUpscale(node, index){
    ...
    try {
        ...
        live.pendingTasks = [{taskId:task.task_id, kind:'image', providerId, model:'', status:'queued', startedAt:nowMs()}];
        live.pending = 1;
        live.running = false;
        render();
        scheduleSave();
...
function runTimePillHtml(node){
    if(!node || node.runTimerHidden || node.type === 'smart-prompt' || node.type === 'smart-video-enhance') return '';
...
function render(){
    ...
    const nodeHtmlEntries = nodes
        ...
        const isMediaKitResult = isSmartMediaKitResultNode(node);
        const displayTitle = node.type === 'smart-video-enhance'
            ? escapeHtml(tr('smart.mediakitTitle'))
            : isMediaKitResult
            ? escapeHtml(tr('smart.mediakitResultTitle'))
            : title;
        ...
        const isMediaKitEnhance = node.type === 'smart-video-enhance';
        ...
        const html = `<div class="image-node ${isEmpty ? 'empty-node' : ''} ${isGroup ? 'group-node' : ''} ${isHistory ? 'history-group-node' : ''} ${isPrompt ? 'prompt-smart-node' : ''} ${isLoop ? 'loop-smart-node' : ''} ${isSmartGroup ? 'smart-group-node' : ''} ${isMediaKitEnhance ? 'mediakit-enhance-node' : ''} ${isMediaKitResult ? 'mediakit-result-node' : ''} ${isCompactMember ? 'smart-group-member-node' : ''} ${isNodeSelected(node.id) ? 'selected' : ''} ${(dragState?.groupIds?.includes(node.id) || dragState?.id === node.id) ? 'dragging' : ''} ${node.running ? 'node-running' : ''} ${isPending ? 'node-pending' : ''}" data-id="${escapeHtml(node.id)}" style="left:${node.x || 0}px;top:${node.y || 0}px;width:${layout.width}px;height:${layout.height}px">
            ...
            ${imgs.length || node.pending || isQueued || isJimengPending || isPrompt || isLoop || isSmartGroup || isMediaKitEnhance || isMediaKitResult ? '<div class="node-resize-handle" data-resize="1"></div>' : ''}
    ...
    [...world.childNodes].forEach(child => {
        if(!keepEls.has(child)){
            pauseMediaInElement(child);
            child.remove();
        }
    });
...
function ensureSingleVideoNodeAspect(node, image){
    if(!node || node.videoAspectInitialized || (node.images || []).length !== 1 || !isVideoMediaItem(image)) return false;
    const size = mediaLayoutSize(image);
    if(size.width <= 0 || size.height <= 0) return false;
    const aspect = size.width / size.height;
    const currentW = Number(node.w);
    if(Number.isFinite(currentW) && currentW > 24){
        node.w = Math.round(currentW);
        node.h = Math.max(48, Math.round(currentW / aspect));
    } else {
        const layout = singleImageLayout(image, {type:node.type, images:node.images}, mediaNodeDefaultScale(node));
        node.w = layout.width;
        node.h = layout.height;
    }
    node.videoAspectInitialized = true;
    return true;
}
function measureSmartNodeImages(){
    world.querySelectorAll('.image-node img,.image-node video').forEach(imgEl => {
        ...
        const image = node?.images?.[index];
        if(imgEl.tagName?.toLowerCase() === 'video') bindSmartVideoPlaybackUi(imgEl);
        if(imgEl.tagName?.toLowerCase() === 'img' && image?.url) bindImageProxyFallback(imgEl, image);
        if(ensureSingleVideoNodeAspect(node, image)){
        ...
        const apply = () => {
            ...
            ensureSingleVideoNodeAspect(node, image);
            applyThumbDisplaySizeToElement(itemEl, image, Math.max(itemEl?.clientWidth || 0, itemEl?.clientHeight || 0));
            updateImageResolutionBadgeElement(itemEl, image);
            if(!isSmartGroupNode(node) && (node.images || []).length === 1 && !node.w && !node.h){
                const layout = singleImageLayout(image, node, mediaNodeDefaultScale(node));
...
function bindNodeEvents(){
    world.querySelectorAll('.image-node').forEach(el => {
        ...
        if(nodeForControls?.type === 'smart-video-enhance') bindSmartMediaKitControls(el, nodeForControls);
        ...
        el.querySelectorAll('[data-mediakit-result-retry]').forEach(btn => {
            btn.addEventListener('click', e => {
                e.preventDefault();
                e.stopPropagation();
                retrySmartMediaKitSave(btn.dataset.mediakitResultRetry || id);
            });
        });
        el.querySelectorAll('[data-mediakit-batch-retry]').forEach(btn => {
            btn.addEventListener('click', e => {
                e.preventDefault();
                e.stopPropagation();
                startSmartMediaKitEnhance(btn.dataset.mediakitBatchRetry || id);
            });
        });
        ...
        el.querySelectorAll('[data-media-task-query]').forEach(btn => {
            btn.addEventListener('mousedown', e => { e.preventDefault(); e.stopPropagation(); }, true);
            btn.addEventListener('click', e => {
                e.preventDefault(); e.stopPropagation();
                querySmartImageTaskNow(btn.dataset.mediaTaskQuery, btn.dataset.taskId);
            });
        });
        ...
        el.querySelectorAll('.smart-video-play').forEach(btn => {
            ...
            btn.addEventListener('click', e => {
                ...
                if(isSmartMediaKitResultNode(nodeForControls)){
                    selectedId = id;
                    selectedIds = [];
        ...
        el.querySelectorAll('.thumb-item,.image-wrap').forEach(item => {
            ...
            item.addEventListener('click', e => {
                ...
                if(mediaKindForItem(target.image || {}) === 'video'){
                    if(isSmartMediaKitResultNode(owner)){
                        selectedId = id;
                        selectedIds = [];
                        selectedImage = {nodeId:target.targetNodeId, index:target.imageIndex};
                        syncSelectionUi();
        ...
        el.querySelector('.node-resize-handle')?.addEventListener('mousedown', e => {
            ...
            const singleVideo = (node.images || []).length === 1 && isVideoMediaItem(node.images[0]);
            if(singleVideo){
                const sourceSize = mediaLayoutSize(node.images[0]);
                resizeState.lockAspect = true;
                resizeState.aspectRatio = sourceSize.width > 0 && sourceSize.height > 0
...
function canAutoConnectDraggedNode(sourceNode, targetNode){
    ...
    if(isSmartVideoEnhanceNode(targetNode)){
        return imagesForNode(sourceNode).some(item => item?.url && mediaKindForItem(item) === 'video');
    }
    if(isSmartVideoEnhanceNode(sourceNode)) return isSmartImageNode(targetNode);
...
function deleteNode(id){
    ...
    const deleting = nodes.find(node => node.id === id);
    const pollOwnerId = isSmartVideoEnhanceNode(deleting)
        ? deleting.id
        : nodes.find(node => isSmartVideoEnhanceNode(node) && node.mediakitResultNodeId === id)?.id;
    if(pollOwnerId){
        smartMediaKitPollers.forEach((entry, key) => {
            if(key.startsWith(`${pollOwnerId}:`)) entry.controller?.abort();
        });
    }
    nodes.forEach(node => {
...
function setComposerMediaKitMode(node){
    const active = Boolean(mediaKitComposerPanel && isSmartMediaKitResultNode(node));
    composer.classList.toggle('mediakit-mode', active);
    if(!mediaKitComposerPanel) return active;
    mediaKitComposerPanel.hidden = !active;
    if(!active){
        mediaKitComposerPanel.innerHTML = '';
        return false;
    }
    mediaKitComposerPanel.innerHTML = smartMediaKitEnhanceBodyHtml(node, {composer:true});
    bindSmartMediaKitControls(mediaKitComposerPanel, node);
    refreshIcons();
    return true;
}
...
function updateComposer(){
    ...
    if(setComposerMediaKitMode(node)){
        if(cascadeRunBtn) cascadeRunBtn.style.display = 'none';
        savePromptDraftForCurrent();
        activeComposerSubject = null;
        lastComposerNodeId = `${node.id}:mediakit`;
...
function connectInputNode(fromId, toId){
    ...
    if(isSmartVideoEnhanceNode(to)){
        const hasVideo = imagesForNode(from).some(item => item?.url && mediaKindForItem(item) === 'video');
        if(!hasVideo) return false;
    }
    if(isSmartVideoEnhanceNode(from) && !isSmartImageNode(to)) return false;
...
function buildPromptRequest(node, overrideDefaultImages=null, consumeDefault=false, ctx=smartLoopContext){
    ...
    parts.forEach(part => {
        ...
        if(!refMap.has(part.url)){
            ...
            refs.push({url:part.url, name:mentionName, nodeId:part.nodeId, imageIndex:part.imageIndex, kind:part.kind || 'image', asset_uris:part.asset_uris || {}, role:`image_${refs.length + 1}`});
        }
...
async function generateUrlsForCurrentSettings(node, prompt, refs, runSettings=settings){
    ...
    if(isApiLikeEngine(activeSettings.engine) && activeSettings.apiKind === 'video'){
        ...
        node.pendingTasks = taskIds.map(taskId => ({
            taskId,
            kind:'video',
        ...
        node.pending = Math.max(taskIds.length, Number(node.pending || 0) || taskIds.length);
        node.running = false;
        render();
        scheduleSave();
        await saveCanvas();
        const settled = await Promise.all(taskIds.map(taskId => pollSmartCanvasTask(taskId, 'video')));
        node.pendingTasks = smartPendingTasks(node).filter(task => !taskIds.includes(task.taskId));
        node.pending = Math.max(0, Number(node.pending || 0) - taskIds.length);
        if(!node.pendingTasks.length) delete node.pendingTasks;
        scheduleSave();
        const urls = settled.flatMap(result => resultMediaUrls(result?.videos?.length ? result.videos : result)).filter(Boolean);
...
async function runLoopRoundIntoSlot(loopNode, rootNode, outputSlot, loopIndex, ctx){
    ...
    try {
        ...
        if(isApiLikeEngine(runSettings.engine) && runSettings.apiKind !== 'video'){
            ...
            outputSlot.pendingTasks = taskIds.map(taskId => ({taskId, kind:'image', providerId:taskResult.providerId, model:taskResult.model, status:'queued', startedAt:nowMs()}));
...
async function runGeneration(){
    ...
    try {
        ...
        if(isApiLikeEngine(settings.engine) && settings.apiKind === 'video'){
            const videoResult = await generateUrlsForCurrentSettings(pendingNode, prompt, refs, settings);
        ...
        if(isApiLikeEngine(settings.engine) || rhModelMode){
            ...
            if(!taskIds.length) throw new Error(tr('smart.errRunFailed'));
            pendingNode.pendingTasks = taskIds.map(taskId => ({taskId, kind:'image', providerId:outImages.providerId, model:outImages.model, status:'queued', startedAt:nowMs()}));
...
async function runApiVideoGeneration(prompt, refs, runSettings=settings){
    ...
    try {
        ...
        return {taskIds:[result.task_id], count:1, providerId:payload.provider_id, model:payload.model};
    } finally {
        transientSmartCloudLinks = [];
    }
}
...
function mediaTaskEndpoint(kind, taskId='', action='get'){
    const base = kind === 'video' ? '/api/canvas-video-tasks' : '/api/canvas-image-tasks';
    if(!taskId) return base;
    return `${base}/${encodeURIComponent(taskId)}${action === 'query' ? '/query' : ''}`;
}
...
async function querySmartImageTaskNow(nodeId, localTaskId){
    ...
    try {
        ...
        const response = await fetch(mediaTaskEndpoint(kind, task.taskId, 'query'), {method:'POST'});
        if(response.ok){
            data = await response.json();
        } else if(response.status === 404 && kind === 'image' && task.recoverTaskId){
            // Compatibility with pendingTasks saved by older versions.
            data = await fetchImageTaskQuery(providerIdForSmartTask(node, task), task.recoverTaskId);
        } else {
            throw new Error(await smartResponseErrorMessage(response, '查询失败'));
        }
        if(data.status === 'succeeded'){
            task.failed = false;
            task.querying = false;
            ...
            const media = kind === 'video'
                ? (result.videos?.length ? result.videos : result)
                : (result.image_items?.length ? result.image_items : (result.images?.length ? result.images : result));
            finalizeSmartPendingTask(node, task.taskId, resultMediaUrls(media), kind);
            render();
            scheduleSave();
            return;
        }
        if(data.status === 'failed'){
            ...
            task.status = data.status || 'reconnecting';
            task.upstreamTaskId = data.upstream_task_id || task.upstreamTaskId || '';
            task.error = data.error || data.message || '';
            toast(task.status === 'reconnecting' ? '网络仍不稳定，后台会继续自动查询' : '任务仍在生成中');
        }
    } catch(e){
        ...
        render();
        scheduleSave();
        if(latest && !['failed','manual'].includes(String(latest.status || ''))){
...
function updateSmartPendingTaskState(taskId, changes={}){
    let changed = false;
    nodes.forEach(node => {
...
async function pollSmartCanvasTask(taskId, kind='image'){
    ...
    const promise = (async () => {
        ...
        for(let i = 0; i < 43200; i++){
            await new Promise(resolve => setTimeout(resolve, Math.min(10000, 2000 + networkFailures * 1000)));
            let task;
            try {
                task = await fetch(mediaTaskEndpoint(kind, taskId)).then(async r => {
                    if(!r.ok){
                        const error = new Error(await smartResponseErrorMessage(r, tr('smart.errRunFailed')));
                        error.localTaskMissing = r.status === 404;
                        throw error;
                    }
                    return r.json();
                });
                ...
                if(e?.localTaskMissing){
                    updateSmartPendingTaskState(taskId, {kind, status:'manual', error:'本地任务记录不可用，已保留节点和任务信息'});
                    throw new ImageTaskRecoverSignal({taskId, recoverTaskId:taskId, kind, message:'本地任务记录不可用，已保留节点和任务信息'});
                }
                networkFailures += 1;
                updateSmartPendingTaskState(taskId, {kind, status:'reconnecting', error:e.message || '网络连接中断'});
                continue;
            }
            updateSmartPendingTaskState(taskId, {
                kind,
                status:task.status || 'processing',
                upstreamTaskId:task.upstream_task_id || '',
                recoverTaskId:task.upstream_task_id || '',
                error:task.error || ''
            });
            ...
            if(task.status === 'manual'){
                throw new ImageTaskRecoverSignal({taskId, recoverTaskId:task.upstream_task_id || taskId, providerId:task.provider_id, kind, message:task.error || '已停止自动查询，可点击立即查询'});
            }
...
function createNodeFromMenu(type){
    ...
    else if(type === 'video-enhance') created = createSmartMediaKitResultNode(null, {point:p});
...
shell.addEventListener('mousedown', e => {
    if(!zoomPreviewState) return;
    if(e.button !== 0) return;
    if(e.target.closest('.composer,.smart-back,.asset-panel,.asset-toggle,.smart-log-toggle,.smart-shortcut-toggle,.smart-snap-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu,.smart-minimap')) return;
    e.preventDefault();
    e.stopPropagation();
}, true);
...
shell.onmousedown = e => {
    if(zoomPreviewState && e.button === 0 && !e.target.closest('.composer,.smart-back,.asset-panel,.asset-toggle,.smart-log-toggle,.smart-shortcut-toggle,.smart-snap-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu,.smart-minimap')) return;
    if(e.target.closest('.image-node,.composer,.smart-back,.asset-panel,.asset-toggle,.smart-log-toggle,.smart-shortcut-toggle,.smart-snap-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.create-menu,.smart-minimap')) return;
    closeCreateMenu();
...
shell.oncontextmenu = e => {
    ...
    if(didPan || e.target.closest('.composer,.smart-back,.asset-panel,.asset-toggle,.smart-log-toggle,.smart-shortcut-toggle,.smart-snap-toggle,.smart-workflow-toggle,.log-modal,.shortcut-modal,.image-edit-modal,.create-menu,.smart-minimap')) return;
...
window.onmousemove = e => {
    ...
    if(resizeState){
        ...
        if(resizeState.lockAspect && resizeState.aspectRatio > 0){
            const aspect = resizeState.aspectRatio;
            const widthChange = Math.abs(dx / Math.max(1, resizeState.startW));
            const heightChange = Math.abs(dy / Math.max(1, resizeState.startH));
            let nextW;
            let nextH;
            if(widthChange >= heightChange){
                nextW = Math.max(minW, resizeState.startW + dx);
                nextH = nextW / aspect;
                if(nextH < minH){ nextH = minH; nextW = nextH * aspect; }
            } else {
                nextH = Math.max(minH, resizeState.startH + dy);
                nextW = nextH * aspect;
                if(nextW < minW){ nextW = minW; nextH = nextW / aspect; }
            }
...
shell.addEventListener('wheel', e => {
    if(e.target.closest('.composer,.smart-back,.image-edit-modal,.asset-panel,.asset-toggle,.smart-log-toggle,.smart-shortcut-toggle,.smart-snap-toggle,.smart-workflow-toggle,.workflow-transfer-panel,.log-modal,.shortcut-modal,.prompt-node-segments,.prompt-node-text,.prompt-node-llm,.smart-group-list,[data-thumb-scroll]')) return;
    e.preventDefault();
    const rect = shell.getBoundingClientRect();
...
if(smartSnapToggle) smartSnapToggle.onclick = event => {
    event.preventDefault();
    event.stopPropagation();
