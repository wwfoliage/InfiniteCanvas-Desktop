...
const canvasMediaKitPollers = new Map();
...
function addVideoEnhanceNode(point){
    const p = point || defaultPoint(180, 0);
    return addNode({
        id:uid('mke'),
        type:'mediakitEnhance',
        x:p.x,
        y:p.y,
        w:380,
        resolution:'720p',
...
function linkCreateOptions(state){
    ...
    if(state.originKind === 'out'){
        if(node.type === 'mediakitEnhance'){
            return [{type:'output', label:'Output', icon:'circle-dot'}];
        }
        if(node.type === 'video'){
            return [
                {type:'mediakitEnhance', label:tr('canvas.mediakitTitle'), icon:'wand-sparkles'},
        ...
        if(['image','prompt','loop','group','promptGroup','llm','output'].includes(node.type)){
            return [
                ...
                ...(['image','group','output'].includes(node.type) ? [{type:'mediakitEnhance', label:tr('canvas.mediakitTitle'), icon:'wand-sparkles'}] : []),
...
function openGeneratorNodeMenu(nodeId, clientX, clientY){
    ...
    const outputOptions = [
        {type:'output', label:'Output', icon:'circle-dot'},
        ...(node.type !== 'mediakitEnhance' && CANVAS_MEDIA_OUTPUT_TYPES.includes(node.type) ? [
            {type:'mediakitEnhance', label:tr('canvas.mediakitTitle'), icon:'wand-sparkles'}
        ] : []),
...
function createNodeByType(type, point){
    ...
    if(type === 'mediakitEnhance') return addVideoEnhanceNode(point);
...
function menuAdd(type){
    ...
    if(type === 'mediakitEnhance') addVideoEnhanceNode(menuPoint);
...
function renderNode(node){
    ...
    const title = node.type === 'image' ? 'Image' : node.type === 'prompt' ? 'Prompt' : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? 'Prompts' : node.type === 'group' ? 'Group' : node.type === 'output' ? 'Output' : node.type === 'llm' ? 'LLM' : node.type === 'comfy' ? 'ComfyUI' : node.type === 'ltxDirector' ? tr('canvas.ltxDirector') : node.type === 'rh' ? 'RunningHub' : node.type === 'msgen' ? tr('canvas.modelscopeGenerate') : node.type === 'video' ? tr('canvas.videoGenerateNode') : node.type === 'mediakitEnhance' ? tr('canvas.mediakitTitle') : tr('canvas.apiGenerate');
    ...
    if(node.type === 'mediakitEnhance') body.appendChild(renderVideoEnhanceBody(node));
    ...
    const canInput = ['generator','comfy','ltxDirector','output','llm','msgen','video','mediakitEnhance','rh'].includes(node.type) || (node.type === 'loop' && (node.imageInput || node.showPrompt));
    const canOutput = ['image','prompt','loop','group','promptGroup','generator','comfy','ltxDirector','llm','msgen','video','mediakitEnhance','rh','output'].includes(node.type);
...
function defaultNodeSize(type){
    ...
    if(type === 'mediakitEnhance') return {w:380, h:0};
...
function canvasMediaKitVideoRefs(node){
    return orderedSources(node, generatorSources(node))
        .flatMap(source => source.refs || [])
        .filter(ref => ref?.url && mediaKindForRef(ref) === 'video');
}
function canvasMediaKitStatusText(node){
    const status = String(node?.mediakitStatus || 'idle');
    if(node?.mediakitMessage) return node.mediakitMessage;
    if(status === 'uploading') return tr('canvas.mediakitUploading');
    if(status === 'submitted' || status === 'processing') return tr('canvas.mediakitProcessing');
    if(status === 'saving') return tr('canvas.mediakitSaving');
    if(status === 'succeeded') return tr('canvas.mediakitSucceeded');
    if(status === 'save_failed') return tr('canvas.mediakitSaveFailed');
    if(status === 'failed') return tr('canvas.mediakitFailed');
    return tr('canvas.mediakitIdle');
}
function renderVideoEnhanceBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'generator-body mediakit-generator-body';
    node.resolution = ['720p','1080p','2k'].includes(String(node.resolution || '').toLowerCase()) ? String(node.resolution).toLowerCase() : '720p';
    node.bitrateLevel = ['low','medium','high'].includes(String(node.bitrateLevel || '').toLowerCase()) ? String(node.bitrateLevel).toLowerCase() : 'medium';
    if(typeof node.keepSourceFps !== 'boolean') node.keepSourceFps = true;
    node.fps = Math.max(15, Math.min(120, Number(node.fps || 30)));
    const refs = canvasMediaKitVideoRefs(node);
    const source = refs.length === 1 ? refs[0] : null;
    const status = String(node.mediakitStatus || 'idle');
    const busy = Boolean(node.running || ['uploading','submitted','processing','saving'].includes(status));
    const statusClass = node.mediakitError ? 'error' : status === 'succeeded' ? 'success' : '';
    const sourceHtml = source
        ? `<div class="mediakit-source-card">${canvasVideoPreviewHtml(source.url, 384)}<div class="mediakit-source-meta"><i data-lucide="video"></i><span>${escapeHtml(source.name || tr('canvas.mediakitVideoInput'))}</span></div></div>`
        : `<div class="mediakit-source-empty"><i data-lucide="video"></i><span>${escapeHtml(refs.length > 1 ? tr('canvas.mediakitOneVideo') : tr('canvas.mediakitConnectVideo'))}</span></div>`;
    wrap.innerHTML = `
        <div class="video-input-head">
            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">${tr('canvas.mediakitVideoInput')}</div>
        </div>
        <div class="mediakit-source-wrap">${sourceHtml}</div>
        <div class="gen-settings">
            <div class="gen-settings-row">
                <div class="field">
                    <div class="setting-title">${tr('canvas.mediakitResolution')}</div>
                    <div class="seg mediakit-param-seg">
                        <button type="button" data-mediakit-resolution="720p" class="${node.resolution === '720p' ? 'active' : ''}">720P</button>
                        <button type="button" data-mediakit-resolution="1080p" class="${node.resolution === '1080p' ? 'active' : ''}">1080P</button>
                        <button type="button" data-mediakit-resolution="2k" class="${node.resolution === '2k' ? 'active' : ''}">2K</button>
                    </div>
                </div>
                <div class="field">
                    <div class="setting-title">${tr('canvas.mediakitBitrate')}</div>
                    <div class="seg mediakit-param-seg">
                        <button type="button" data-mediakit-bitrate="low" class="${node.bitrateLevel === 'low' ? 'active' : ''}">${tr('canvas.mediakitLow')}</button>
                        <button type="button" data-mediakit-bitrate="medium" class="${node.bitrateLevel === 'medium' ? 'active' : ''}">${tr('canvas.mediakitMedium')}</button>
                        <button type="button" data-mediakit-bitrate="high" class="${node.bitrateLevel === 'high' ? 'active' : ''}">${tr('canvas.mediakitHigh')}</button>
                    </div>
                </div>
            </div>
            <div class="gen-settings-row mediakit-fps-row">
                <button type="button" class="setting-check ${node.keepSourceFps ? 'active' : ''}" data-mediakit-keep-fps><span class="check-dot"></span>${tr('canvas.mediakitKeepFps')}</button>
                <label class="field mediakit-fps-field ${node.keepSourceFps ? 'is-hidden' : ''}">
                    <div class="setting-title">${tr('canvas.mediakitTargetFps')}</div>
                    <input class="setting-input mediakit-fps" type="number" min="15" max="120" step="1" value="${node.fps}">
                </label>
            </div>
        </div>
        <div class="mediakit-node-status ${statusClass}">
            <i data-lucide="${busy ? 'loader-2' : node.mediakitError ? 'circle-alert' : status === 'succeeded' ? 'circle-check' : 'info'}"></i>
            <span>${escapeHtml(node.mediakitError || canvasMediaKitStatusText(node))}</span>
        </div>
        <div class="gen-run-row">
            <button class="gen-btn mediakit-run ${busy ? 'running' : ''}" ${busy || !source ? 'disabled' : ''}><i data-lucide="wand-sparkles" class="w-4 h-4"></i>${busy ? tr('canvas.mediakitRunning') : tr('canvas.mediakitStart')}</button>
            ${status === 'save_failed' && node.mediakitTaskId ? `<button type="button" class="gen-cascade-btn mediakit-retry-save"><i data-lucide="download" class="w-4 h-4"></i><span>${tr('canvas.mediakitRetrySave')}</span></button>` : ''}
        </div>
    `;
    const fps = wrap.querySelector('.mediakit-fps');
    wrap.querySelectorAll('[data-mediakit-resolution]').forEach(button => {
        button.onclick = e => {
            e.stopPropagation();
            node.resolution = button.dataset.mediakitResolution;
            node.mediakitError = '';
            render();
            scheduleSave();
        };
    });
    wrap.querySelectorAll('[data-mediakit-bitrate]').forEach(button => {
        button.onclick = e => {
            e.stopPropagation();
            node.bitrateLevel = button.dataset.mediakitBitrate;
            node.mediakitError = '';
            render();
            scheduleSave();
        };
    });
    wrap.querySelector('[data-mediakit-keep-fps]').onclick = e => {
        e.stopPropagation();
        node.keepSourceFps = !node.keepSourceFps;
        node.mediakitError = '';
        render();
        scheduleSave();
    };
    if(fps){
        fps.onchange = e => {
            node.fps = Math.max(15, Math.min(120, Math.round(Number(e.target.value || 30))));
            e.target.value = String(node.fps);
            node.mediakitError = '';
            scheduleSave();
        };
    }
    wrap.querySelector('.mediakit-run').onclick = e => { e.stopPropagation(); runCanvasGenerate(node.id); };
    wrap.querySelector('.mediakit-retry-save')?.addEventListener('click', e => {
        e.stopPropagation();
        retryCanvasMediaKitSave(node.id);
    });
    return wrap;
}
function canvasMediaKitOutputNode(node){
    return nodes.find(item => item.id === node?.mediakitOutputNodeId && item.type === 'output')
        || connections.filter(c => c.from === node?.id).map(c => nodes.find(item => item.id === c.to)).find(item => item?.type === 'output')
        || null;
}
function canvasMediaKitPending(out, nodeId){
    return (out?._pending || []).find(item => item.canvasTaskType === 'mediakit-video' && item.mediaKitEnhanceNodeId === nodeId) || null;
}
function ensureCanvasMediaKitPending(out, node, run=null){
    if(!out || !node) return null;
    const existing = canvasMediaKitPending(out, node.id);
    if(existing) return existing;
    const pending = makePending(`mediakit:${node.id}:${node.mediakitClientToken || Date.now()}`, run || runSnapshot(node, '', canvasMediaKitVideoRefs(node)), {
        canvasTaskType:'mediakit-video',
        mediaKitTaskId:node.mediakitTaskId || '',
        mediaKitEnhanceNodeId:node.id,
        previewSize:{w:16, h:9}
    });
    out._pending = [...(out._pending || []), pending];
    return pending;
}
function applyCanvasMediaKitTask(node, task){
    if(!node || !task) return null;
    const status = String(task.status || node.mediakitStatus || 'processing');
    node.mediakitTaskId = task.task_id || node.mediakitTaskId || '';
    node.mediakitStatus = status;
    node.mediakitMessage = task.message || '';
    node.mediakitError = task.error || '';
    const out = canvasMediaKitOutputNode(node);
    const pending = canvasMediaKitPending(out, node.id);
    if(pending && node.mediakitTaskId) pending.mediaKitTaskId = node.mediakitTaskId;
    if(status === 'succeeded' && task.result?.url){
        ...
        if(out){
            out._pending = (out._pending || []).filter(entry => entry !== pending);
            const run = pending?.run || runSnapshot(node, '', canvasMediaKitVideoRefs(node));
            const runMs = Math.max(0, nowMs() - Number(pending?.startedAt || nowMs()));
            appendOutputImagesWithoutDuplicates(out, [item], canvasMediaKitVideoRefs(node)[0], [{run, runMs, kind:'video'}]);
            if(node.mediakitLoggedTaskId !== node.mediakitTaskId){
                addGenerationLog({run, outputs:[item], runMs});
                node.mediakitLoggedTaskId = node.mediakitTaskId;
            }
        }
        mergeGeneratedOutputs(node, [item], false);
        node.running = false;
        node.runStatus = 'done';
        node.runError = '';
    } else if(status === 'failed' || status === 'save_failed'){
        if(out) out._pending = (out._pending || []).filter(entry => entry !== pending);
        node.running = false;
        node.runStatus = 'failed';
        node.runError = node.mediakitError || canvasMediaKitStatusText(node);
    } else {
        node.running = true;
    }
    refreshNodes([node.id, out?.id].filter(Boolean));
    scheduleSave();
    return status;
}
async function pollCanvasMediaKitTask(nodeId, taskId){
    if(!taskId || canvasMediaKitPollers.has(taskId)) return;
    const controller = new AbortController();
    canvasMediaKitPollers.set(taskId, controller);
    try {
        const task = await window.MediaKitClient.pollTask(taskId, {
            signal:controller.signal,
            onUpdate:update => {
                const live = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
                if(live) applyCanvasMediaKitTask(live, update);
            }
        });
        const live = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
        if(live) applyCanvasMediaKitTask(live, task);
        return task;
    } catch(error) {
        if(error?.name === 'AbortError') return null;
        const live = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
        if(live && live.mediakitTaskId === taskId){
            live.running = false;
            live.mediakitError = window.MediaKitClient.errorMessage(error, tr('canvas.mediakitQueryFailed'));
            refreshNodes([live.id]);
            scheduleSave();
            setTimeout(() => {
                const retryNode = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
                if(retryNode?.mediakitTaskId === taskId && !window.MediaKitClient.TERMINAL_STATUSES.has(String(retryNode.mediakitStatus || ''))){
                    pollCanvasMediaKitTask(nodeId, taskId);
                }
            }, 5000);
        }
        return null;
    } finally {
        canvasMediaKitPollers.delete(taskId);
    }
}
async function runVideoEnhanceNode(nodeId, opts={}){
    const node = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
    if(!node || node.running) return;
    const refs = canvasMediaKitVideoRefs(node);
    const fail = message => {
        node.mediakitError = message;
        node.mediakitStatus = 'failed';
        node.runStatus = 'failed';
        node.runError = message;
        refreshNodes([node.id]);
        scheduleSave();
        if(opts.cascade) throw new Error(message);
        showErrorModal(message, tr('canvas.mediakitTitle'));
    };
    if(refs.length !== 1) return fail(refs.length > 1 ? tr('canvas.mediakitOneVideo') : tr('canvas.mediakitConnectVideo'));
    if(!window.MediaKitClient) return fail(tr('canvas.mediakitClientMissing'));
    try {
        const settings = await window.MediaKitClient.getSettings();
        if(!settings.configured) return fail(tr('canvas.mediakitConfigureKey'));
        const out = outputForNode(node, 430);
        if(!out) return fail(tr('canvas.mediakitOutputRequired'));
        const canReuseClientToken = node.mediakitStatus === 'failed'
            && !node.mediakitTaskId
            && Boolean(node.mediakitClientToken)
            && /响应缺少 result|未返回任务 ID|未返回 task_id/.test(String(node.mediakitError || ''));
        const clientToken = canReuseClientToken
            ? node.mediakitClientToken
            : window.MediaKitClient.createClientToken();
        node.mediakitClientToken = clientToken;
        node.mediakitTaskId = '';
        node.mediakitOutputNodeId = out.id;
        node.mediakitStatus = 'uploading';
        node.mediakitMessage = tr('canvas.mediakitPreparing');
        node.mediakitError = '';
        node.running = true;
        node.runStatus = 'running';
        node.runError = '';
        const run = runSnapshot(node, '', refs);
        const pending = ensureCanvasMediaKitPending(out, node, run);
        render();
        scheduleSave();
        clearTimeout(saveTimer);
        saveTimer = null;
        ...
        try {
            metadata = await Promise.race([
                window.MediaKitClient.videoMetadata(refs[0].url),
                new Promise(resolve => setTimeout(() => resolve(metadata), 5000))
            ]);
        } catch(error) {}
        const task = await window.MediaKitClient.createEnhanceTask({
            video:{...refs[0], ...metadata},
            resolution:node.resolution,
            bitrate_level:node.bitrateLevel,
            fps:node.keepSourceFps ? null : node.fps,
            keepSourceFps:node.keepSourceFps,
            client_token:clientToken,
            canvas_surface:'classic',
            canvas_id:canvas?.id || '',
            enhance_node_id:node.id,
            result_node_id:out.id
        });
        node.mediakitTaskId = task.task_id || '';
        if(pending) pending.mediaKitTaskId = node.mediakitTaskId;
        applyCanvasMediaKitTask(node, task);
        clearTimeout(saveTimer);
        saveTimer = null;
        await saveCanvas();
        if(node.mediakitTaskId && !window.MediaKitClient.TERMINAL_STATUSES.has(String(task.status || ''))){
            return await pollCanvasMediaKitTask(node.id, node.mediakitTaskId);
        }
        return task;
    } catch(error) {
        if(error?.name === 'AbortError') return;
        const out = canvasMediaKitOutputNode(node);
        if(out) out._pending = (out._pending || []).filter(item => item.mediaKitEnhanceNodeId !== node.id);
        return fail(window.MediaKitClient?.errorMessage(error, tr('canvas.mediakitFailed')) || tr('canvas.mediakitFailed'));
    }
}
async function retryCanvasMediaKitSave(nodeId){
    const node = nodes.find(item => item.id === nodeId && item.type === 'mediakitEnhance');
    if(!node?.mediakitTaskId || !window.MediaKitClient) return;
    const out = canvasMediaKitOutputNode(node) || outputForNode(node, 430);
    node.mediakitStatus = 'saving';
    node.mediakitMessage = tr('canvas.mediakitSaving');
    node.mediakitError = '';
    node.running = true;
    ensureCanvasMediaKitPending(out, node);
    refreshNodes([node.id, out?.id].filter(Boolean));
    scheduleSave();
    try {
        const task = await window.MediaKitClient.retrySave(node.mediakitTaskId);
        applyCanvasMediaKitTask(node, task);
        if(!window.MediaKitClient.TERMINAL_STATUSES.has(String(task.status || ''))){
            await pollCanvasMediaKitTask(node.id, node.mediakitTaskId);
        }
    } catch(error) {
        node.running = false;
        node.mediakitStatus = 'save_failed';
        node.mediakitError = window.MediaKitClient.errorMessage(error, tr('canvas.mediakitSaveFailed'));
        if(out) out._pending = (out._pending || []).filter(item => item.mediaKitEnhanceNodeId !== node.id);
        refreshNodes([node.id, out?.id].filter(Boolean));
        scheduleSave();
    }
}
function resumeCanvasMediaKitTasks(){
    if(!window.MediaKitClient) return;
    nodes.filter(node => node.type === 'mediakitEnhance' && node.mediakitTaskId).forEach(node => {
        const status = String(node.mediakitStatus || '');
        const out = canvasMediaKitOutputNode(node);
        const alreadyComplete = status === 'succeeded' && (out?.images || []).some(item => item?.mediakitTaskId === node.mediakitTaskId);
        ...
        if(!window.MediaKitClient.TERMINAL_STATUSES.has(status)) ensureCanvasMediaKitPending(out, node);
        pollCanvasMediaKitTask(node.id, node.mediakitTaskId);
    });
}
...
const CANVAS_GENERATOR_TYPES = ['generator','msgen','comfy','ltxDirector','video','mediakitEnhance','rh'];
const CANVAS_IMAGE_OUTPUT_TYPES = ['generator','msgen','comfy','ltxDirector','rh'];
const CANVAS_MEDIA_OUTPUT_TYPES = ['generator','msgen','comfy','ltxDirector','video','mediakitEnhance','rh'];
...
function shouldCreateOutputForNode(node){
    if(!node) return false;
    if(node.type === 'mediakitEnhance') return true;
...
function refreshDownstreamVideoEnhancers(sourceId){
    const ids = connections
        .filter(connection => connection.from === sourceId)
        .map(connection => nodes.find(node => node.id === connection.to))
        .filter(node => node?.type === 'mediakitEnhance')
        .map(node => node.id);
    if(ids.length) setTimeout(() => refreshNodes(ids), 0);
}
function generatedImageRefs(node){
    const keepGeneratedMedia = ['rh','ltxDirector','video','mediakitEnhance'].includes(node?.type);
...
function runCascadeNodeByType(node, opts={}){
    ...
    if(node.type === 'mediakitEnhance') return runVideoEnhanceNode(node.id, runOpts);
...
function canvasRunTypes(){
    return ['generator','msgen','comfy','ltxDirector','llm','video','mediakitEnhance','rh'];
}
...
function deleteNode(id, event){
    ...
    destroyLTXEditor(deletingNode);
    if(deletingNode?.type === 'mediakitEnhance' && deletingNode.mediakitTaskId){
        canvasMediaKitPollers.get(deletingNode.mediakitTaskId)?.abort();
        canvasMediaKitPollers.delete(deletingNode.mediakitTaskId);
    }
...
function runTaskLabel(run){
    ...
    if(run?.nodeType === 'mediakitEnhance') return 'MediaKit Video Enhance';
...
function runPlatformLabel(run){
    ...
    if(run?.nodeType === 'mediakitEnhance') return 'Volcengine MediaKit';
...
function mergeGeneratedOutputs(node, outputs, append=false){
    ...
    if(!append){
        ...
        refreshDownstreamVideoEnhancers(node.id);
        return;
    }
...
function appendOutputImages(out, images, compareRef, metas=[], layout=null){
    ...
    out.images = [...(out.images || []), ...list.map((url, i) => {
        ...
        if(source.mediakitTaskId) item.mediakitTaskId = source.mediakitTaskId;
    ...
    refreshDownstreamVideoEnhancers(out.id);
}
...
function canConnect(fromId, toId){
    ...
    if(to.type === 'mediakitEnhance'){
        const existingInput = connections.find(c => c.to === toId && c.from !== fromId);
        if(existingInput) return false;
        const videoCapable = from.type === 'image'
            || from.type === 'group'
            || from.type === 'output'
            || CANVAS_MEDIA_OUTPUT_TYPES.includes(from.type);
        return videoCapable && !wouldCreateGeneratorCycle(fromId, toId);
    }
    if(from.type === 'mediakitEnhance'){
...
function deleteSelectedNodes(){
    ...
    toDelete.forEach(id => {
        const deletingNode = nodes.find(n => n.id === id && n.type === 'mediakitEnhance');
        if(deletingNode?.mediakitTaskId){
            canvasMediaKitPollers.get(deletingNode.mediakitTaskId)?.abort();
            canvasMediaKitPollers.delete(deletingNode.mediakitTaskId);
        }
    });
