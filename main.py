...
async def startup_event():
    ...
    try:
        await resume_canvas_media_tasks()
    except Exception as exc:
        print(f"恢复画布媒体任务失败: {exc}")
...
OUTPUT_INPUT_DIR = str(APP_PATHS.asset_input_dir)
OUTPUT_OUTPUT_DIR = str(APP_PATHS.asset_output_dir)
ASSET_LIBRARY_DIR = str(APP_PATHS.asset_library_dir)
LOCAL_UPLOAD_DIR = str(APP_PATHS.local_upload_dir)
HISTORY_FILE = str(APP_PATHS.history_file)
API_ENV_FILE = str(APP_PATHS.api_env_file)
DATA_DIR = str(APP_PATHS.runtime_data_dir)
CONVERSATION_DIR = str(APP_PATHS.conversation_dir)
CANVAS_DIR = str(APP_PATHS.canvas_dir)
MEDIA_PREVIEW_DIR = str(APP_PATHS.media_preview_dir)
ASSET_LIBRARY_PATH = str(APP_PATHS.asset_library_file)
PROMPT_LIBRARY_PATH = str(APP_PATHS.prompt_library_file)
API_PROVIDERS_FILE = str(APP_PATHS.api_providers_file)
RUNNINGHUB_WORKFLOW_STORE_FILE = str(APP_PATHS.runninghub_workflow_store_file)
SHARED_FOLDERS_FILE = str(APP_PATHS.shared_folders_file)
...
STORAGE_SETTINGS_FILE = str(APP_PATHS.storage_settings_file)
MEDIAKIT_SETTINGS_FILE = str(APP_PATHS.mediakit_settings_file)
MEDIAKIT_TASKS_FILE = str(APP_PATHS.mediakit_tasks_file)
MEDIAKIT_API_BASE = "https://mediakit.cn-beijing.volces.com/api/v1"
MEDIAKIT_PROCESS_ID = uuid.uuid4().hex
MEDIAKIT_DEFAULT_OUTPUT_SUBDIR = "mediakit-enhance"
MEDIAKIT_SUPPORTED_EXTS = {".mp4", ".flv", ".ts", ".avi", ".mov", ".wmv", ".mkv"}
MEDIAKIT_MAX_LOCAL_BYTES = 5 * 1024 * 1024 * 1024
MEDIAKIT_RESOLUTIONS = {"720p", "1080p", "2k"}
MEDIAKIT_BITRATE_LEVELS = {"low", "medium", "high"}
...
def default_mediakit_settings():
    return {
        "output_mode": "generated",
        "custom_output_dir": "",
    }

def normalize_mediakit_settings(raw=None):
    raw = raw if isinstance(raw, dict) else {}
    mode = str(raw.get("output_mode") or "generated").strip().lower()
    if mode not in {"generated", "custom"}:
        mode = "generated"
    custom = str(raw.get("custom_output_dir") or "").strip().strip('"')
    if custom:
        custom = os.path.abspath(os.path.expanduser(os.path.expandvars(custom)))
    return {
        "output_mode": mode,
        "custom_output_dir": custom,
    }

def load_mediakit_settings():
    try:
        with open(MEDIAKIT_SETTINGS_FILE, "r", encoding="utf-8-sig") as handle:
            return normalize_mediakit_settings(json.load(handle))
    except (FileNotFoundError, json.JSONDecodeError, OSError, UnicodeError):
        return default_mediakit_settings()
...
def mediakit_output_dir(settings=None, ensure=False):
    settings = normalize_mediakit_settings(settings or load_mediakit_settings())
    if settings["output_mode"] == "custom":
        root = settings["custom_output_dir"]
        if not root or not os.path.isabs(root):
            raise HTTPException(status_code=400, detail="MediaKit 自定义保存位置必须是绝对路径")
    else:
        root = os.path.join(OUTPUT_OUTPUT_DIR, MEDIAKIT_DEFAULT_OUTPUT_SUBDIR)
    root = os.path.abspath(root)
    if ensure:
        try:
            os.makedirs(root, exist_ok=True)
            probe = os.path.join(root, f".mediakit-write-{uuid.uuid4().hex}.tmp")
            with open(probe, "wb") as handle:
                handle.write(b"ok")
            os.remove(probe)
        except OSError as exc:
            raise HTTPException(status_code=400, detail=f"MediaKit 保存位置不可写：{exc}")
    return root

def save_mediakit_settings(raw):
    settings = normalize_mediakit_settings(raw)
    mediakit_output_dir(settings, ensure=True)
    atomic_write_json(MEDIAKIT_SETTINGS_FILE, settings)
    return settings

def mediakit_key_value():
    return strip_auth_scheme(provider_env_key_value("mediakit"), "Bearer")

def public_mediakit_settings():
    settings = load_mediakit_settings()
    key = mediakit_key_value()
    return {
        **settings,
        "configured": bool(key),
        "key_preview": mask_secret(key),
        "resolved_output_dir": mediakit_output_dir(settings),
    }
...
SUPPORTED_PROVIDER_PROTOCOLS = {"openai", "apimart", "gemini", "grok", "gemini-cli", "volcengine", "runninghub", "jimeng", "codex", "mediakit"}
...
def merge_default_api_providers(providers, inject_missing=True):
    ...
    mediakit_default = next((d for d in default_api_providers() if d["id"] == "mediakit"), None)
    if mediakit_default:
        current = next((item for item in merged if item.get("id") == "mediakit"), None)
        if not current:
            merged.append(mediakit_default)
        else:
            current["name"] = "AI MediaKit"
            current["base_url"] = MEDIAKIT_API_BASE
            current["protocol"] = "mediakit"
...
def provider_endpoint_url(provider, key, default_path):
    ...
    if override:
        ...
        return override
    api_prefixes = ("/api/v3", "/v1beta", "/v1", "/v2")
    base_prefix = next((prefix for prefix in api_prefixes if base_url.endswith(prefix)), "")
    path_prefix = next((prefix for prefix in api_prefixes if default_path.startswith(f"{prefix}/")), "")
    if base_prefix and path_prefix:
        if base_prefix == path_prefix:
            return f"{base_url}{default_path[len(path_prefix):]}"
        # A provider can expose OpenAI-compatible and Gemini-native models under
        # different API versions. Replace the configured trailing version instead
        # of producing an invalid URL such as /v1/v1beta/models/...
        base_url = base_url[:-len(base_prefix)].rstrip("/")
...
def normalize_provider(item):
    ...
    if provider_id == "mediakit":
        protocol = "mediakit"
        base_url = MEDIAKIT_API_BASE
        item = {**item, "primary": False, "image_models": [], "chat_models": [], "video_models": []}
...
def get_primary_provider_id(providers=None):
    ...
    primary = next((p for p in providers if p.get("id") != "mediakit" and p.get("primary") and p.get("enabled", True)), None)
    if primary:
        return primary["id"]
    non_ms = next((p for p in providers if p["id"] not in {"modelscope", "mediakit"} and p.get("enabled", True)), None)
    if non_ms:
        return non_ms["id"]
    fallback = next((p for p in providers if p.get("id") != "mediakit"), None)
    return fallback["id"] if fallback else "modelscope"
...
CANVAS_MEDIA_TASKS_FILE = os.path.join(DATA_DIR, "canvas_media_tasks.json")
CANVAS_MEDIA_TASK_STORE = CanvasMediaTaskStore(CANVAS_MEDIA_TASKS_FILE, recovery_seconds=24 * 60 * 60)
CANVAS_MEDIA_WORKERS: Dict[str, asyncio.Task] = {}
CANVAS_MEDIA_WORKERS_LOCK = Lock()


def report_canvas_media_task(task_id: str, **changes):
    """Persist a media task transition and mirror it for legacy callers."""
    if not task_id:
        return {}
    try:
        task = CANVAS_MEDIA_TASK_STORE.patch(task_id, **changes)
    except KeyError:
        task = {"id": task_id, **changes, "updated_at": time.time()}
    with CANVAS_TASK_LOCK:
        existing = CANVAS_TASKS.setdefault(task_id, {})
        existing.update(task)
    return task


def canvas_media_task(task_id: str):
    task = CANVAS_MEDIA_TASK_STORE.get(task_id)
    if task:
        return task
    with CANVAS_TASK_LOCK:
        found = CANVAS_TASKS.get(task_id)
        return dict(found) if found else None


def start_canvas_media_worker(task_id: str):
    """Start at most one recovery worker for a persisted local task."""
    with CANVAS_MEDIA_WORKERS_LOCK:
        current = CANVAS_MEDIA_WORKERS.get(task_id)
        if current and not current.done():
            return current
        task = canvas_media_task(task_id) or {}
        kind = str(task.get("kind") or "image")
        worker_coro = recover_canvas_video_task(task_id) if kind == "video" else recover_canvas_image_task(task_id)
        worker = asyncio.create_task(worker_coro)
        CANVAS_MEDIA_WORKERS[task_id] = worker

        def clear_worker(done_task):
            with CANVAS_MEDIA_WORKERS_LOCK:
                if CANVAS_MEDIA_WORKERS.get(task_id) is done_task:
                    CANVAS_MEDIA_WORKERS.pop(task_id, None)

        worker.add_done_callback(clear_worker)
        return worker


async def resume_canvas_media_tasks():
    """Resume live tasks after a local service restart without creating new IDs."""
    CANVAS_MEDIA_TASK_STORE.mark_expired()
    CANVAS_MEDIA_TASK_STORE.cleanup()
    for task in CANVAS_MEDIA_TASK_STORE.recoverable():
        with CANVAS_TASK_LOCK:
            CANVAS_TASKS[task["id"]] = dict(task)
        start_canvas_media_worker(task["id"])
...
class MediaKitVideoInput(BaseModel):
    url: str = ""
    name: str = ""
...
class MediaKitEnhanceRequest(BaseModel):
    video: MediaKitVideoInput
    resolution: str = "720p"
...
def is_apimart_provider(provider):
    ...
    return provider_protocol(provider) == "apimart" or "apimart.ai" in base_url or "api.apib.ai" in base_url


TRANSIENT_MEDIA_STATUS = {408, 425, 429, 500, 502, 503, 504, 520, 522, 524}
FATAL_MEDIA_STATUS = {400, 401, 402, 403, 404, 409, 422}


def media_task_error_disposition(exc):
    """Classify whether durable media work should reconnect or stop."""
    ...
    try:
        ...
        status_code = None
    if status_code in TRANSIENT_MEDIA_STATUS:
        return "retry"
    if status_code in FATAL_MEDIA_STATUS:
        return "fail"
    return "retry" if isinstance(exc, (OSError, ConnectionError)) else "fail"


def media_task_retry_delay(attempt, retry_after=""):
    ...
    if retry_after_value > 0:
        return min(60.0, max(1.0, retry_after_value))
    try:
        attempt_value = max(0, int(attempt))
    except (TypeError, ValueError):
        attempt_value = 0
    base = min(60.0, 3.0 * (1.7 ** attempt_value))
    return min(60.0, base + random.uniform(0, min(2.0, base * 0.1)))


def media_task_submit_headers(provider, local_task, json_body=True, model=""):
    """Build submit headers and add a stable key only where supported."""
    headers = api_headers(json_body=json_body, provider=provider, model=model)
    if is_apimart_provider(provider):
        idempotency_key = str((local_task or {}).get("idempotency_key") or "").strip()
        if idempotency_key:
...
@app.get("/api/mediakit/settings")
async def get_mediakit_settings():
    return public_mediakit_settings()

@app.patch("/api/mediakit/settings")
async def update_mediakit_settings(payload: Dict[str, Any]):
    payload = payload or {}
    env_updates = {}
    if bool(payload.get("clear_api_key")):
        env_updates[provider_key_env("mediakit")] = ""
    else:
        api_key = str(payload.get("api_key") or "").strip()
        if api_key:
            env_updates[provider_key_env("mediakit")] = strip_auth_scheme(api_key, "Bearer")
    current = load_mediakit_settings()
    next_settings = {
        "output_mode": payload.get("output_mode", current["output_mode"]),
        "custom_output_dir": payload.get("custom_output_dir", current["custom_output_dir"]),
    }
    save_mediakit_settings(next_settings)
    if env_updates:
        update_env_values(env_updates)
    return public_mediakit_settings()

@app.post("/api/mediakit/open-output-dir")
async def open_mediakit_output_dir():
    path = mediakit_output_dir(ensure=True)
    try:
        if os.name == "nt":
            os.startfile(path)
        ...
        raise HTTPException(status_code=500, detail=f"无法打开 MediaKit 保存位置：{exc}")
    return {"ok": True}
...
def load_mediakit_tasks():
    try:
        with open(MEDIAKIT_TASKS_FILE, "r", encoding="utf-8-sig") as handle:
    ...
    return {"tasks": [item for item in (tasks or []) if isinstance(item, dict)]}

def save_mediakit_tasks(data):
    payload = {"tasks": list((data or {}).get("tasks") or [])[-2000:]}
    atomic_write_json(MEDIAKIT_TASKS_FILE, payload)
    return payload

def mediakit_find_task(local_task_id="", client_token=""):
    local_task_id = str(local_task_id or "").strip()
    client_token = str(client_token or "").strip()
    with MEDIAKIT_TASKS_LOCK:
        data = load_mediakit_tasks()
        for item in data["tasks"]:
            if local_task_id and item.get("id") == local_task_id:
                return dict(item)
            if client_token and item.get("client_token") == client_token:
                return dict(item)
    return None

def mediakit_update_task(local_task_id, **changes):
    with MEDIAKIT_TASKS_LOCK:
        data = load_mediakit_tasks()
        task = next((item for item in data["tasks"] if item.get("id") == local_task_id), None)
        if not task:
            raise HTTPException(status_code=404, detail="MediaKit 任务不存在")
        task.update(changes)
        task["updated_at"] = now_ms()
        save_mediakit_tasks(data)
        return dict(task)

def mediakit_create_task(record):
    with MEDIAKIT_TASKS_LOCK:
        data = load_mediakit_tasks()
        token = str(record.get("client_token") or "")
        existing = next((item for item in data["tasks"] if item.get("client_token") == token), None)
        if existing:
...
def public_mediakit_task(task):
    task = task if isinstance(task, dict) else {}
    result = None
    local_url = str(task.get("asset_url") or task.get("result_url") or "").strip()
    if local_url:
        result = {
            "url": local_url,
            "saved_url": str(task.get("result_url") or local_url),
            "name": str(task.get("result_name") or ""),
            "kind": "video",
    ...
    return {
        ...
        "client_token": str(task.get("client_token") or ""),
        "status": str(task.get("status") or "unknown"),
        "message": str(task.get("message") or ""),
        "error": str(task.get("error") or ""),
        "canvas_surface": str(task.get("canvas_surface") or ""),
        "canvas_id": str(task.get("canvas_id") or ""),
        "enhance_node_id": str(task.get("enhance_node_id") or ""),
        "result_node_id": str(task.get("result_node_id") or ""),
...
def mediakit_safe_text(value, fallback="MediaKit 请求失败"):
    text = re.sub(r"(?i)bearer\s+[^\s,;]+", "Bearer [redacted]", str(value or "")).strip()
    text = re.sub(r"https?://\S+", "[remote URL]", text)
    return (text or fallback)[:500]

def mediakit_validate_client_token(value):
    token = str(value or "").strip() or f"mk_{uuid.uuid4().hex}"
    if len(token) > 64 or not re.fullmatch(r"[\x21-\x7e]+", token):
        raise HTTPException(status_code=400, detail="MediaKit client_token 必须是 1–64 个可打印 ASCII 字符")
    return token

def mediakit_validate_request(payload):
    resolution = str(payload.resolution or "720p").strip().lower()
    bitrate = str(payload.bitrate_level or "medium").strip().lower()
    if resolution not in MEDIAKIT_RESOLUTIONS:
        raise HTTPException(status_code=400, detail="目标分辨率只支持 720p、1080p 或 2K")
    if bitrate not in MEDIAKIT_BITRATE_LEVELS:
        raise HTTPException(status_code=400, detail="码率档位只支持低、中或高")
    fps = payload.fps
    if fps is not None:
        try:
            fps = int(fps)
        except (TypeError, ValueError):
            raise HTTPException(status_code=400, detail="目标帧率必须是 15–120 的整数")
        if fps < 15 or fps > 120:
            raise HTTPException(status_code=400, detail="目标帧率必须在 15–120 之间")
    video_url = str(payload.video.url or "").strip()
    if not video_url:
        raise HTTPException(status_code=400, detail="请先连接一个视频")
    width = int(payload.video.width or 0)
    height = int(payload.video.height or 0)
    if width > 0 and height > 0:
        short_edge, long_edge = sorted((width, height))
        if short_edge < 360 or short_edge > 1080 or long_edge < 360 or long_edge > 1920:
            raise HTTPException(status_code=400, detail="输入视频尺寸超出 MediaKit 支持范围：短边 360–1080，长边 360–1920")
    return {
        "resolution": resolution,
        "bitrate_level": bitrate,
        "fps": fps,
    }

def mediakit_detect_hdr_marker(path):
    try:
        size = os.path.getsize(path)
        ...
        return ""
    for marker in (b"dvcC", b"dvvC", b"dvwC"):
        if marker in sample:
            return "Dolby Vision"
    start = 0
    while True:
        pos = sample.find(b"nclx", start)
        if pos < 0:
            break
        if pos + 8 <= len(sample):
            transfer = int.from_bytes(sample[pos + 6:pos + 8], "big")
            if transfer == 16:
...
def mediakit_local_input_path(video_url):
    text = str(video_url or "").strip()
    parsed = urllib.parse.urlsplit(text)
    if parsed.scheme in {"http", "https"} and (parsed.hostname or "").lower() in {"localhost", "127.0.0.1", "::1"}:
        text = parsed.path + (f"?{parsed.query}" if parsed.query else "")
    try:
        path = local_media_path_from_url(text)
    except HTTPException:
        path = None
    if path:
        return path
    if text.startswith("/"):
        raise HTTPException(status_code=400, detail="找不到要增强的本地视频文件")
    return None

def mediakit_validate_local_input(path):
    ext = os.path.splitext(path)[1].lower()
    if ext not in MEDIAKIT_SUPPORTED_EXTS:
        raise HTTPException(status_code=400, detail="MediaKit 不支持这个视频格式")
    try:
        size = os.path.getsize(path)
    except OSError:
        raise HTTPException(status_code=404, detail="找不到要增强的本地视频文件")
    if size <= 0:
        raise HTTPException(status_code=400, detail="输入视频是空文件")
    if size > MEDIAKIT_MAX_LOCAL_BYTES:
        raise HTTPException(status_code=413, detail="本地视频超过 MediaKit 5 GB 上传限制")
    hdr = mediakit_detect_hdr_marker(path)
    if hdr:
        raise HTTPException(status_code=400, detail=f"MediaKit 生成式视频增强只支持 SDR；检测到 {hdr}")
    return path

def mediakit_remote_input_url(video_url):
    text = str(video_url or "").strip()
    parsed = urllib.parse.urlsplit(text)
    if parsed.scheme.lower() not in {"http", "https", "mediakit", "vod", "tos"}:
        raise HTTPException(status_code=400, detail="视频地址必须是画布本地视频、HTTP(S) 或 MediaKit/VOD/TOS 地址")
    return text

def mediakit_response_objects(payload):
    if not isinstance(payload, dict):
        return []
    objects = [payload]
...
def mediakit_response_error_message(payload, fallback="MediaKit 请求失败"):
    if not isinstance(payload, dict):
        return fallback
    ...
    if isinstance(error, dict):
        for key in ("message", "error", "msg", "detail"):
            ...
            if value:
                return mediakit_safe_text(value, fallback)
    elif isinstance(error, str) and error.strip():
        return mediakit_safe_text(error, fallback)
    for item in mediakit_response_objects(payload):
...
def mediakit_response_payload(response, action):
    try:
        payload = response.json()
    except Exception:
        payload = {}
    if response.status_code >= 400:
        fallback = f"{action}失败（HTTP {response.status_code}）"
        raise HTTPException(status_code=502, detail=mediakit_response_error_message(payload, fallback))
    if not isinstance(payload, dict):
        raise HTTPException(status_code=502, detail=f"{action}返回了无效响应")
    if payload.get("success") is False:
        raise HTTPException(status_code=502, detail=mediakit_response_error_message(payload, f"{action}失败"))
    code = payload.get("code")
    if code not in (None, 0, "0"):
        raise HTTPException(status_code=502, detail=mediakit_response_error_message(payload, f"{action}失败"))
    return payload

def mediakit_response_result(response, action):
    payload = mediakit_response_payload(response, action)
...
def mediakit_response_task_id(payload):
    for item in mediakit_response_objects(payload):
...
def mediakit_response_request_id(payload):
    for item in mediakit_response_objects(payload):
        value = str(item.get("request_id") or "").strip()
        if value:
            return value
    return ""

def mediakit_auth_headers(key):
    return {
        "Authorization": f"Bearer {key}",
        "Content-Type": "application/json",
    }

async def mediakit_upload_local_video(client, key, path):
    response = await client.post(
        f"{MEDIAKIT_API_BASE}/tools-sync/request-media-upload-url",
        headers=mediakit_auth_headers(key),
        json={},
    )
    upload = mediakit_response_result(response, "获取 MediaKit 上传地址")
    upload_url = str(upload.get("upload_url") or "").strip()
    file_id = str(upload.get("file_id") or "").strip()
    method = str(upload.get("method") or "PUT").strip().upper()
    header_items = upload.get("upload_headers") or []
    upload_headers = {}
    for item in header_items:
        if isinstance(item, dict) and str(item.get("key") or "").strip():
            upload_headers[str(item["key"]).strip()] = str(item.get("value") or "")
    if not upload_url or not file_id or method != "PUT":
        raise HTTPException(status_code=502, detail="MediaKit 上传地址响应不完整")
    if not any(key_name.lower() == "content-type" for key_name in upload_headers):
        upload_headers["Content-Type"] = mimetypes.guess_type(path)[0] or "application/octet-stream"
    ...
    if upload_response.status_code < 200 or upload_response.status_code >= 300:
        raise HTTPException(status_code=502, detail=f"上传视频到 MediaKit 失败（HTTP {upload_response.status_code}）")
    return file_id

def mediakit_result_video_url(remote):
    for item in mediakit_response_objects(remote):
        value = str(item.get("video_url") or item.get("url") or "").strip()
        if value:
            return value
    return ""

def mediakit_remote_status(remote):
    for item in mediakit_response_objects(remote):
...
def mediakit_remote_message(remote, fallback=""):
    for item in mediakit_response_objects(remote):
        ...
        error = item.get("error")
        if isinstance(error, dict):
            value = str(error.get("message") or error.get("msg") or "").strip()
        else:
...
def mediakit_can_retry_submit(task):
    task = task if isinstance(task, dict) else {}
    if str(task.get("status") or "").lower() != "failed":
        return False
    if str(task.get("remote_task_id") or "").strip():
        return False
    error = str(task.get("error") or "")
...
def mediakit_result_extension(url, content_type=""):
    ext = os.path.splitext(urllib.parse.urlsplit(str(url or "")).path)[1].lower()
    if ext in MEDIAKIT_SUPPORTED_EXTS:
        return ext
    guessed = mimetypes.guess_extension(str(content_type or "").split(";", 1)[0].strip()) or ""
    return guessed if guessed in MEDIAKIT_SUPPORTED_EXTS else ".mp4"

def mediakit_safe_result_name(source_name, local_task_id, ext):
    stem = os.path.splitext(os.path.basename(str(source_name or "")))[0]
    stem = re.sub(r'[<>:"/\\|?*\x00-\x1f]+', "_", stem).strip(" ._")[:80] or "enhanced-video"
    return f"{stem}_enhanced_{str(local_task_id)[:8]}{ext}"

def mediakit_result_url_for(path):
    root = os.path.abspath(OUTPUT_OUTPUT_DIR)
    path = os.path.abspath(path)
    try:
        if os.path.commonpath([root, path]) == root:
        ...
        pass
    return f"/api/mediakit/files/{urllib.parse.quote(os.path.basename(path), safe='')}"

def register_mediakit_asset(path, display_name, local_task_id):
    lib = load_asset_library()
    library = find_asset_library(lib)
    if not library:
        raise RuntimeError("素材库不存在")
    categories = library.setdefault("categories", [])
    category = next((item for item in categories if item.get("id") == "mediakit-videos"), None)
    if not category:
        category = {
            "id": "mediakit-videos",
            "name": "MediaKit 视频",
            "type": "video",
            "dir": unique_asset_category_dir(library, "MediaKit 视频"),
            "items": [],
        }
        categories.append(category)
    category.setdefault("items", [])
    if not category.get("dir"):
        category["dir"] = unique_asset_category_dir(library, "MediaKit 视频")
    _, item = make_asset_library_item(path, display_name, str(category.get("dir") or ""))
    item["source"] = "mediakit"
    item["mediakit_task_id"] = local_task_id
...
async def mediakit_download_result(client, task, remote_url):
    output_dir = mediakit_output_dir(ensure=True)
    part_path = ""
    try:
        async with client.stream("GET", remote_url, follow_redirects=True) as response:
            if response.status_code < 200 or response.status_code >= 300:
                raise HTTPException(status_code=502, detail=f"下载 MediaKit 结果失败（HTTP {response.status_code}）")
            ext = mediakit_result_extension(remote_url, response.headers.get("content-type", ""))
            filename = mediakit_safe_result_name(task.get("input_name"), task.get("id"), ext)
            ...
            if not os.path.isfile(part_path) or os.path.getsize(part_path) <= 0:
                raise HTTPException(status_code=502, detail="MediaKit 返回了空视频")
            os.replace(part_path, final_path)
        result_url = mediakit_result_url_for(final_path)
        asset_item = None
        asset_error = ""
        try:
            asset_item = await asyncio.to_thread(
            ...
            asset_error = mediakit_safe_text(exc, "素材库登记失败")
        return {
            "result_name": filename,
            "result_url": result_url,
            "asset_url": str((asset_item or {}).get("url") or ""),
            "asset_id": str((asset_item or {}).get("id") or ""),
...
async def submit_mediakit_enhance_task(payload):
    key = mediakit_key_value()
    if not key:
        raise HTTPException(status_code=400, detail="请先在 API 设置中配置 AI MediaKit API Key")
    params = mediakit_validate_request(payload)
    client_token = mediakit_validate_client_token(payload.client_token)
    existing = mediakit_find_task(client_token=client_token)
    if existing:
        if not mediakit_can_retry_submit(existing):
            return public_mediakit_task(existing)
        local_task_id = str(existing.get("id") or "")
        task = mediakit_update_task(
            local_task_id,
            status="uploading",
            message="正在使用原请求凭证安全重试",
            error="",
            input_url=str(payload.video.url or "").strip(),
            input_name=str(payload.video.name or "").strip(),
            params=params,
            canvas_surface=re.sub(r"[^a-zA-Z0-9_-]", "", str(payload.canvas_surface or ""))[:30],
            canvas_id=str(payload.canvas_id or "")[:120],
            enhance_node_id=str(payload.enhance_node_id or "")[:120],
            result_node_id=str(payload.result_node_id or "")[:120],
        )
    else:
        local_task_id = f"mk_{uuid.uuid4().hex}"
        timestamp = now_ms()
        record = {
            "id": local_task_id,
        ...
        task, created = mediakit_create_task(record)
        if not created:
            return public_mediakit_task(task)
    try:
        local_path = mediakit_local_input_path(payload.video.url)
        if local_path:
            mediakit_validate_local_input(local_path)
        else:
            remote_input = mediakit_remote_input_url(payload.video.url)
        timeout = httpx.Timeout(60.0, read=600.0, write=600.0, pool=60.0)
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            if local_path:
                mediakit_update_task(local_task_id, status="uploading", message="正在上传视频")
                remote_input = await mediakit_upload_local_video(client, key, local_path)
            request_body = {
                "video_url": remote_input,
                "resolution": params["resolution"],
                "bitrate_level": params["bitrate_level"],
                "client_token": client_token,
            }
            if params["fps"] is not None:
                request_body["fps"] = params["fps"]
            response = await client.post(
                f"{MEDIAKIT_API_BASE}/tools/enhance-video-generative",
                headers=mediakit_auth_headers(key),
                json=request_body,
            )
            response_payload = mediakit_response_payload(response, "创建 MediaKit 增强任务")
        remote_task_id = mediakit_response_task_id(response_payload)
        if not remote_task_id:
            raise HTTPException(status_code=502, detail="MediaKit 创建成功响应未返回 task_id")
        task = mediakit_update_task(
            local_task_id,
            status="submitted",
            message="已提交，等待 MediaKit 处理",
            remote_task_id=remote_task_id,
            remote_request_id=mediakit_response_request_id(response_payload),
        )
        return public_mediakit_task(task)
    except HTTPException as exc:
        mediakit_update_task(local_task_id, status="failed", message="", error=mediakit_safe_text(exc.detail))
        raise
    except Exception as exc:
        detail = mediakit_safe_text(exc)
        mediakit_update_task(local_task_id, status="failed", message="", error=detail)
        raise HTTPException(status_code=502, detail=detail)

async def query_mediakit_enhance_task(local_task_id):
    task = mediakit_find_task(local_task_id=local_task_id)
    if not task:
        raise HTTPException(status_code=404, detail="MediaKit 任务不存在")
    if task.get("status") in {"succeeded", "failed", "save_failed"}:
        return public_mediakit_task(task)
    if task.get("status") == "saving":
        if str(task.get("saving_owner") or "") == MEDIAKIT_PROCESS_ID:
            return public_mediakit_task(task)
        if str(task.get("remote_result_url") or "").strip():
            mediakit_update_task(
                local_task_id,
                status="save_failed",
                message="检测到服务重启，正在恢复本地保存",
                error="",
                saving_owner="",
            )
            return await retry_mediakit_result_save(local_task_id)
        return public_mediakit_task(task)
    ...
    key = mediakit_key_value()
    if not key:
        raise HTTPException(status_code=400, detail="MediaKit API Key 已移除，无法继续查询任务")
    timeout = httpx.Timeout(60.0, read=600.0, write=600.0, pool=60.0)
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            response = await client.get(
                f"{MEDIAKIT_API_BASE}/tasks/{urllib.parse.quote(remote_task_id, safe='')}",
                headers=mediakit_auth_headers(key),
            )
            remote = mediakit_response_payload(response, "查询 MediaKit 任务")
            status = mediakit_remote_status(remote)
            if status in {"failed", "error", "cancelled", "canceled"}:
                reason = mediakit_remote_message(remote, "MediaKit 处理失败")
                task = mediakit_update_task(local_task_id, status="failed", message="", error=mediakit_safe_text(reason))
                return public_mediakit_task(task)
            remote_url = mediakit_result_video_url(remote)
            if status in {"completed", "succeeded", "success", "done"} or remote_url:
                if not remote_url:
                    raise HTTPException(status_code=502, detail="MediaKit 任务完成但没有返回视频地址")
                with MEDIAKIT_TASKS_LOCK:
                    latest = mediakit_find_task(local_task_id=local_task_id)
                    if latest.get("status") == "succeeded":
                        return public_mediakit_task(latest)
                    if latest.get("status") == "saving" and str(latest.get("saving_owner") or "") == MEDIAKIT_PROCESS_ID:
                        return public_mediakit_task(latest)
                    task = mediakit_update_task(
                        local_task_id,
                        status="saving",
                        message="增强完成，正在保存到本地",
                        remote_result_url=remote_url,
                        error="",
                        saving_owner=MEDIAKIT_PROCESS_ID,
                    )
                try:
                    saved = await mediakit_download_result(client, task, remote_url)
                    task = mediakit_update_task(
                        local_task_id,
                        status="succeeded",
                        message="画质增强完成",
                    ...
                    task = mediakit_update_task(
                        local_task_id,
                        status="save_failed",
                        message="增强完成，但保存到本地失败",
                        error=mediakit_safe_text(detail),
                        remote_result_url=remote_url,
                        saving_owner="",
                    )
                return public_mediakit_task(task)
            task = mediakit_update_task(
                local_task_id,
                status="processing",
                message=mediakit_remote_message(remote, "MediaKit 正在增强视频")[:200],
                error="",
            )
            return public_mediakit_task(task)
    except HTTPException:
        ...
        raise HTTPException(status_code=502, detail=mediakit_safe_text(exc))

async def retry_mediakit_result_save(local_task_id):
    task = mediakit_find_task(local_task_id=local_task_id)
    if not task:
        raise HTTPException(status_code=404, detail="MediaKit 任务不存在")
    if task.get("status") == "succeeded":
        return public_mediakit_task(task)
    remote_url = str(task.get("remote_result_url") or "").strip()
    if task.get("status") != "save_failed" or not remote_url:
        raise HTTPException(status_code=400, detail="这个任务当前不能重试保存")
    task = mediakit_update_task(
        local_task_id,
        status="saving",
        message="正在重新保存增强视频",
...
def mediakit_file_path(filename):
    value = urllib.parse.unquote(str(filename or "")).replace("\\", "/")
    if not value or value != os.path.basename(value) or value in {".", ".."}:
        raise HTTPException(status_code=400, detail="非法 MediaKit 文件路径")
    root = mediakit_output_dir()
...
@app.post("/api/mediakit/enhance-tasks")
async def create_mediakit_enhance_task(payload: MediaKitEnhanceRequest):
    return await submit_mediakit_enhance_task(payload)

@app.get("/api/mediakit/enhance-tasks/{local_task_id}")
async def get_mediakit_enhance_task(local_task_id: str):
    return await query_mediakit_enhance_task(local_task_id)

@app.post("/api/mediakit/enhance-tasks/{local_task_id}/retry-save")
async def retry_mediakit_enhance_save(local_task_id: str):
    return await retry_mediakit_result_save(local_task_id)

@app.get("/api/mediakit/files/{filename:path}")
async def get_mediakit_result_file(filename: str):
    path = mediakit_file_path(filename)
    if not os.path.isfile(path):
        raise HTTPException(status_code=404, detail="MediaKit 视频不存在")
    return FileResponse(path, media_type=content_type_for_path(path))
...
async def upload_video_for_apimart(client, provider, ref_url: str) -> str:
    ...
    优先使用已有公网地址；本地视频自动上传到临时云存储。未经官方文档确认的
    APIMart 视频上传端点仅在 APIMART_TRY_VIDEO_UPLOAD 显式开启时作为回退。"""
    ref_url = str(ref_url or "").strip()
    if not ref_url:
        return "ERR:空地址"
    ...
    cloud_error = ""
    try:
        uploaded = await upload_local_video_to_cloud(ref_url, "auto")
        cloud_url = str((uploaded or {}).get("url") or "").strip()
        if valid_apimart_video_image_input(cloud_url):
            return cloud_url
        cloud_error = "临时存储没有返回可用的公网 URL"
    except HTTPException as exc:
        cloud_error = str(exc.detail or "临时存储上传失败")
    except Exception as exc:
        cloud_error = f"临时存储上传异常：{exc}"
    if str(os.getenv("APIMART_TRY_VIDEO_UPLOAD") or "").strip().lower() not in {"1", "true", "yes", "on"}:
        return f"ERR:本地参考视频自动上传失败：{cloud_error}"
    base_url = video_api_root(provider)
    ...
    return (
        f"ERR:本地参考视频自动上传失败：{cloud_error}；"
        f"APIMart 实验上传入口也不可用（{last_error}）。"
        "请改用公网 http/https / asset:// 视频地址。"
    )
...
async def generate_gemini_provider_image(prompt, size, model, reference_images=None, provider=None):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=1800.0, write=120.0, pool=20.0)) as client:
        ...
        try:
            ...
            debug_path = os.path.join(debug_dir, f"gemini_response_{uuid.uuid4().hex}.json")
            try:
                with open(debug_path, "w", encoding="utf-8") as _f:
                    json.dump(raw, _f, ensure_ascii=False, indent=2)
                print(f"[DEBUG] Gemini 响应已保存至: {debug_path}")
            except Exception:
                pass
            raise
...
async def generate_ai_image(prompt, size, quality, model, reference_images=None, provider_id="comfly", aspect_ratio="", resolution="", local_task_id=""):
    ...
    local_submit_task = canvas_media_task(local_task_id) if local_task_id else {}

    def submit_headers(json_body=True):
        if local_submit_task and is_apimart:
            return media_task_submit_headers(
                provider, local_submit_task, json_body=json_body, model=model
            )
        return api_headers(json_body=json_body, provider=provider, model=model)

    def report_upstream_task(upstream_task_id):
        upstream_task_id = str(upstream_task_id or "").strip()
        if local_task_id and upstream_task_id:
            report_canvas_media_task(
                local_task_id,
                status="submitted",
    ...
    async with httpx.AsyncClient(timeout=request_timeout) as client:
        ...
        if image_request_mode == "openai-video-proxy":
            ...
            if has_local_images:
                ...
                headers = submit_headers(json_body=False)
            ...
            response = await client.post(gen_url, headers=submit_headers(), json=body)
        elif is_apimart:
...
async def save_providers(payload: List[ApiProviderPayload]):
    ...
    primary_indices = [
        i for i, flag in enumerate(raw_primary_flags)
        if flag and i < len(providers) and providers[i].get("id") != "mediakit"
    ]
    if primary_indices:
        winner = primary_indices[-1]
        for i, p in enumerate(providers):
            p["primary"] = (i == winner and p.get("id") != "mediakit")
...
async def build_online_image_result(payload: OnlineImageRequest, local_task_id=""):
    ...
    try:
        ...
        wrapped = HTTPException(status_code=502, detail=f"请求上游生图接口失败：{exc}")
        upstream_task_id = getattr(exc, "upstream_task_id", "")
        if upstream_task_id:
            setattr(wrapped, "upstream_task_id", upstream_task_id)
...
async def run_canvas_image_task(
    task_id: str, payload: OnlineImageRequest, schedule_recovery: bool = True
):
    report_canvas_media_task(task_id, status="running", error="")
    try:
        result = await build_online_image_result(payload, local_task_id=task_id)
        report_canvas_media_task(
        ...
        provider = get_api_provider(payload.provider_id)
        retryable = media_task_error_disposition(exc) == "retry"
        can_resubmit = bool(upstream_task_id) or is_apimart_provider(provider)
        if retryable and can_resubmit:
            report_canvas_media_task(
                task_id,
                status="reconnecting",
                error=str(detail),
            ...
            if schedule_recovery:
                start_canvas_media_worker(task_id)
        elif retryable:
            report_canvas_media_task(
                task_id,
                status="manual",
                error=f"网络中断，平台不支持安全重提；请手动查询原任务。{detail}",
                status_code=status_code,
                upstream_task_id=upstream_task_id,
            )
        else:
...
async def recover_canvas_image_task(task_id: str, force_once: bool = False):
    """Reconnect to the same image job until success or the 24-hour deadline."""
    while True:
        task = canvas_media_task(task_id)
        if not task or task.get("status") in {"succeeded", "failed"}:
            return task
        deadline = float(task.get("recovery_deadline") or 0)
        if not force_once and deadline and time.time() > deadline:
            report_canvas_media_task(task_id, status="manual")
            return canvas_media_task(task_id)
        upstream_task_id = str(task.get("upstream_task_id") or "").strip()
        provider_id = str(task.get("provider_id") or "comfly")
        if not upstream_task_id:
            ...
            if not is_apimart_provider(provider):
                report_canvas_media_task(
                    task_id,
                    status="manual",
                    error="本地服务在取得上游任务号前中断，当前平台不支持安全重提。",
                )
                return canvas_media_task(task_id)
            try:
                payload = OnlineImageRequest(**(task.get("payload") or {}))
                await run_canvas_image_task(
                    task_id, payload, schedule_recovery=False
                )
            except Exception as exc:
                report_canvas_media_task(
                    task_id, status="reconnecting", error=str(exc)
                )
            refreshed = canvas_media_task(task_id) or {}
            if refreshed.get("status") in {"succeeded", "failed", "manual"}:
                return refreshed
            if force_once:
                return refreshed
            await asyncio.sleep(media_task_retry_delay(refreshed.get("attempt", 0)))
            report_canvas_media_task(
                task_id, attempt=int(refreshed.get("attempt") or 0) + 1
            )
            continue
        try:
            result = await query_image_task(
                ImageTaskQueryRequest(
                    provider_id=provider_id, task_id=upstream_task_id
                )
            )
            status = str((result or {}).get("status") or "running")
            if status == "succeeded":
                return report_canvas_media_task(
                    task_id, status="succeeded", result=result, error=""
                )
            if status == "failed":
                return report_canvas_media_task(
                    task_id,
                    status="failed",
                    error=str((result or {}).get("error") or "上游生图任务失败"),
                )
            report_canvas_media_task(
                task_id, status="processing", error="", upstream_task_id=upstream_task_id
            )
        except Exception as exc:
            if media_task_error_disposition(exc) == "fail":
                return report_canvas_media_task(
                    task_id,
                    status="failed",
        ...
        if force_once:
            return canvas_media_task(task_id)
        current = canvas_media_task(task_id) or {}
        attempt = int(current.get("attempt") or 0) + 1
        report_canvas_media_task(task_id, attempt=attempt)
        await asyncio.sleep(media_task_retry_delay(attempt))
...
async def create_canvas_image_task(payload: OnlineImageRequest):
    ...
    default_model = (provider.get("image_models") or [IMAGE_MODEL])[0]
    model = selected_model(payload.model, default_model)
    task = CANVAS_MEDIA_TASK_STORE.create(
        "image",
        provider["id"],
        model,
        payload.dict(),
    ...
    task_id = task["id"]
    task = report_canvas_media_task(task_id, type="online-image")
    with CANVAS_TASK_LOCK:
        CANVAS_TASKS[task_id] = dict(task)
...
async def get_canvas_image_task(task_id: str):
    task = canvas_media_task(task_id) or {}
    if not task:
        raise HTTPException(status_code=404, detail="画布任务不存在或任务记录已清理")
    return task


@app.post("/api/canvas-image-tasks/{task_id}/query")
async def query_canvas_image_task_now(task_id: str):
    task = canvas_media_task(task_id)
    if not task or task.get("kind") != "image":
        raise HTTPException(status_code=404, detail="画布生图任务不存在")
    if task.get("status") in {"succeeded", "failed"}:
        return task
    report_canvas_media_task(task_id, status="reconnecting", error="")
    await recover_canvas_image_task(task_id, force_once=True)
    refreshed = canvas_media_task(task_id) or {}
    if refreshed.get("status") not in {"succeeded", "failed"}:
        start_canvas_media_worker(task_id)
    return refreshed
...
async def fetch_video_task_payload_once(client, provider, task_id, submit_url=""):
    """Query an existing upstream video job once without creating a new job."""
    base_url = video_api_root(provider)
    task_urls = video_task_url_candidates(provider, base_url, task_id, submit_url)
    last_error = None
    for task_url in task_urls:
        try:
            response = await client.get(
                task_url, headers=api_headers(provider=provider)
            )
            response.raise_for_status()
            return response.json()
        except Exception as exc:
...
def video_task_status_and_reason(raw):
    raw = raw if isinstance(raw, dict) else {}
    task_data = (
        raw.get("data")
...
async def build_canvas_video_result(payload: CanvasVideoRequest, local_task_id=""):
    provider = get_api_provider(payload.provider_id)
    if is_jimeng_provider(provider):
    ...
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            ...
            for idx, candidate_url in enumerate(submit_urls):
                ...
                if is_apimart and local_submit_task:
                    response = await httpx_request_with_transient_retries(
                        client,
                        "POST",
                        submit_url,
                        attempts=2,
                        headers=media_task_submit_headers(
                            provider, local_submit_task, model=requested_model
                        ),
                        json=body,
                    )
                else:
                    response = await client.post(
                        submit_url, headers=api_headers(provider=provider), json=body
                    )
            ...
            if task_id and not video_output_urls(raw):
                try:
                    result = await wait_for_video_task(
                        client, provider, task_id, submit_url
                    )
                except (HTTPException, httpx.HTTPError) as exc:
                    setattr(exc, "upstream_task_id", task_id)
                    raise
        ...
        wrapped = HTTPException(status_code=502, detail=f"请求上游视频接口失败：{exc}")
        upstream_task_id = getattr(exc, "upstream_task_id", "")
        if upstream_task_id:
            setattr(wrapped, "upstream_task_id", upstream_task_id)
        raise wrapped from exc
...
async def run_canvas_video_task(
    task_id: str, payload: CanvasVideoRequest, schedule_recovery: bool = True
):
    report_canvas_media_task(task_id, status="running", error="")
    try:
        result = await build_canvas_video_result(payload, local_task_id=task_id)
        report_canvas_media_task(
            task_id, status="succeeded", result=result, error=""
        )
    except Exception as exc:
        ...
        upstream_task_id = (
            ...
            or (canvas_media_task(task_id) or {}).get("upstream_task_id")
...
async def query_existing_video_task(task):
    """Query and save an existing upstream result; never submit generation."""
    provider = get_api_provider(task.get("provider_id") or "comfly")
    upstream_task_id = str(task.get("upstream_task_id") or "").strip()
    if not upstream_task_id:
        return {"status": "running"}
    timeout = httpx.Timeout(connect=20.0, read=300.0, write=60.0, pool=20.0)
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        raw = await fetch_video_task_payload_once(
            client,
            provider,
            upstream_task_id,
            str(task.get("submit_url") or ""),
        )
        status, reason = video_task_status_and_reason(raw)
        urls = video_output_urls(raw)
        if status in VIDEO_TASK_FAILURE_STATUSES:
            return {
                "status": "failed",
                "error": humanize_video_task_failure(reason),
                "task_id": upstream_task_id,
                "raw": raw,
            }
        if status in VIDEO_TASK_SUCCESS_STATUSES or urls:
            if not urls:
                raise HTTPException(
                    status_code=502,
                    detail=f"视频任务已完成但没有返回视频：{raw}",
                )
            # If saving fails, the durable task remains attached to this upstream ID.
            # The next recovery pass retries only these downloads.
            local_urls = []
            for url in urls:
                local_urls.append(await save_remote_video_to_output(url))
            return {
                "status": "succeeded",
                "videos": local_urls,
                "task_id": upstream_task_id,
...
async def recover_canvas_video_task(task_id: str, force_once: bool = False):
    """Reconnect to the same video task without duplicating generation."""
    while True:
        task = canvas_media_task(task_id)
        if not task or task.get("status") in {"succeeded", "failed"}:
            return task
        ...
        if not upstream_task_id:
            ...
            if not is_apimart_provider(provider):
                ...
                return canvas_media_task(task_id)
            payload = CanvasVideoRequest(**(task.get("payload") or {}))
            await run_canvas_video_task(
                task_id, payload, schedule_recovery=False
            )
            refreshed = canvas_media_task(task_id) or {}
            if refreshed.get("status") in {"succeeded", "failed", "manual"}:
            ...
            try:
                result = await query_existing_video_task(task)
                ...
                if status == "failed":
                    return report_canvas_media_task(
                        task_id,
                        status="failed",
                        error=str(result.get("error") or "上游视频任务失败"),
                    )
                report_canvas_media_task(
                    task_id,
                    status="processing",
...
async def create_canvas_video_task(payload: CanvasVideoRequest):
    ...
    model = selected_model(payload.model, "veo3-fast")
    task = CANVAS_MEDIA_TASK_STORE.create(
        "video", provider["id"], model, payload.dict()
    )
    task_id = task["id"]
    task = report_canvas_media_task(task_id, type="online-video")
    with CANVAS_TASK_LOCK:
        CANVAS_TASKS[task_id] = dict(task)
...
async def get_canvas_video_task(task_id: str):
    task = canvas_media_task(task_id)
    if not task or task.get("kind") != "video":
        raise HTTPException(status_code=404, detail="画布视频任务不存在或任务记录已清理")
    return task


@app.post("/api/canvas-video-tasks/{task_id}/query")
async def query_canvas_video_task_now(task_id: str):
    task = canvas_media_task(task_id)
    if not task or task.get("kind") != "video":
        raise HTTPException(status_code=404, detail="画布视频任务不存在")
    if task.get("status") in {"succeeded", "failed"}:
        return task
    report_canvas_media_task(task_id, status="reconnecting", error="")
    await recover_canvas_video_task(task_id, force_once=True)
    refreshed = canvas_media_task(task_id) or {}
    if refreshed.get("status") not in {"succeeded", "failed"}:
        start_canvas_media_worker(task_id)
    return refreshed
